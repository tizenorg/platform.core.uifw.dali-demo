...
int main( int argc, char **argv )
{
  ...
  //float downscale = optionParser.GetDownscale();
  float outputScale = optionParser.GetOutputScale();
  Vector2 framebufferSize = optionParser.GetFramebufferSize();
  ...
  {
    //Vector2 framebufferSize = Vector2( image.GetImageWidth(), image.GetImageHeight()) * downscale;
    Vector2 outputSize = Vector2( image.GetImageWidth(), image.GetImageHeight()) * outputScale ;
    SuperBlurGenerator blurGenerator( image, outputFolderPath, outputFileName, blurLevel, framebufferSize, outputSize);
