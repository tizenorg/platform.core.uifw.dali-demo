...
public:
   ...
   * level - 1-6
   */
  SuperBlurGeneratorImpl( const Dali::BitmapLoader& inputImage, const std::string& outputFolder, const std::string& outputFileName, int level, float downscalingFactor )
  : mWidth(inputImage.GetImageWidth()),
    mHeight(inputImage.GetImageHeight()),
    mDownscaledWidth(mWidth * downscalingFactor),
    mDownscaledHeight(mHeight * downscalingFactor),
    mContext(),
    ...
    mKernel(mNumSamples, BellCurveWidth(level-1), mWidth, mHeight),
  ...
  void Generate()
  {
    ...
    Framebuffer fb1( mDownscaledWidth, mDownscaledHeight);
    {
      fb1.Prepare();
      mBlurProgram.Activate();
      mKernel.Generate( mWidth, mHeight );
    ...
    mKernel.Generate( mDownscaledWidth, mDownscaledHeight );
...
private:
  std::size_t mWidth;
  std::size_t mHeight;
  std::size_t mDownscaledWidth;
  std::size_t mDownscaledHeight;
...
SuperBlurGenerator::SuperBlurGenerator(
  ...
  int blurLevel )
: mImpl( new SuperBlurGeneratorImpl( inputImage, outputFolder, outputFileName, blurLevel, 0.5f ) )
{
}
