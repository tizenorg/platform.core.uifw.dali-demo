...
const Vector2 POSITION_SWING_3DEFFECT( 0.45f, 0.4f );             ///< Position Swing relative to stage size.
...
{
  return Vector3( ScalePointSize( vec.x ), ScalePointSize( vec.y ), ScalePointSize( vec.z ) );
}
...
void DaliTableView::Initialize( Application& application )
{
  ...
  const float toolbarHeight = DP( DemoHelper::DEFAULT_VIEW_STYLE.mToolBarHeight );
