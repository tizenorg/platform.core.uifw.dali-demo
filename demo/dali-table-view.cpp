...
void DaliTableView::AddBackgroundActors( Actor layer, int count, BufferImage distanceField )
{
  for( int i = 0; i < count; ++i )
  {
    ...
    Toolkit::DistanceFieldEffectSetProperties( dfActor );
    dfActor.SetColor( BUBBLE_COLOR[ i%NUMBER_OF_BUBBLE_COLOR ] );
    dfActor.SetProperty( dfActor.GetPropertyIndex( "uOutlineParams" ), Vector2( 0.55f, 0.00f ) );
    dfActor.SetProperty( dfActor.GetPropertyIndex( "uSmoothing" ), 0.5f );
    layer.Add( dfActor );
  }
