...
void DaliTableView::Initialize( Application& application )
{
  ...
  alignmentActor.SetPadding( Actor::Padding( 0.0f, 0.0f, logoMargin, logoMargin ));
  ...
  mScrollView.SetPadding( Actor::Padding( buttonsPageMargin, buttonsPageMargin, 0.0f, 0.0f ) );
    ...
    for( int t = 0; t < mTotalPages; t++ )
    {
      ...
      for(int row = 0; row < ROWS_PER_PAGE; row++)
      {
        for(int column = 0; column < EXAMPLES_PER_ROW; column++)
        {
          ...
          tile.SetPadding( Actor::Padding( margin, margin, margin, margin ) );
