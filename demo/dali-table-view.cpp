...
ImageActor CreateBackground( std::string imagePath )
{
  ...
  background.SetResizePolicy( FILL_TO_PARENT );
...
private:
  ...
  float mLateralDirection;
  float mLateralOffset;
};
...
void DaliTableView::Initialize( Application& application )
{
  ...
  mRootActor.SetResizePolicy( FILL_TO_PARENT );
  ...
  alignment.SetScaling( Alignment::ScaleNone );
  Actor alignmentActor = alignment;
  alignmentActor.SetPadding( Vector2( logoMargin, logoMargin ), HEIGHT );
  ...
  mScrollView.SetResizePolicy( FILL_TO_PARENT );
  const float buttonsPageMargin = ( 1.0f - TABLE_RELATIVE_SIZE.x ) * 0.5f * stageSize.width;
  mScrollView.SetPadding( Vector2( buttonsPageMargin, buttonsPageMargin ), WIDTH );
  ...
  mScrollViewLayer.SetResizePolicy( FILL_TO_PARENT );
  ...
  backgroundColourActor.SetResizePolicy( FILL_TO_PARENT );
  ...
  bubbleContainer.SetResizePolicy( FILL_TO_PARENT );
  ...
  buttonsAlignment.SetResizePolicy( FILL_TO_PARENT );
...
void DaliTableView::Populate()
{
  ...
  if( mExampleList.size() > 0 )
  {
    ...
    for( int t = 0; t < mTotalPages; t++ )
    {
      ...
      page.SetResizePolicy( FILL_TO_PARENT );
      ...
      for(int row = 0; row < ROWS_PER_PAGE; row++)
      {
        for(int column = 0; column < EXAMPLES_PER_ROW; column++)
        {
          ...
          tile.SetPadding( Vector2( margin, margin ) );
...
Actor DaliTableView::CreateTile( const std::string& name, const std::string& title, const Dali::Vector3& sizeMultiplier, bool addBackground )
{
  ...
  content.SetResizePolicy( FILL_TO_PARENT );
  content.SetRelayoutSizeFactor( sizeMultiplier );
  ...
  if( addBackground )
  {
    ...
    image.SetResizePolicy( FILL_TO_PARENT );
    ...
    stencil.SetResizePolicy( FILL_TO_PARENT );
    image.Add( stencil );
  }
  ...
  text.SetResizePolicy( FILL_TO_PARENT );
  text.SetRelayoutSizeFactor( Vector3( 0.9f, 0.9f, 0.0f ) );
...
void DaliTableView::InitialiseBackgroundActors( Actor actor )
{
  ...
  for( unsigned int i = 0, childCount = actor.GetChildCount(); i < childCount; ++i )
  {
    ...
    Constraint animConstraint = Constraint::New < Vector3 > ( Dali::Actor::Property::Position,
