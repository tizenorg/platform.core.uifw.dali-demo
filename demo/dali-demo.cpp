...
private:
  ...
  PanInfo PredictionCodeTest1( unsigned int lastVSyncTime, unsigned int nextVSyncTime )
  {
    ...
    unsigned int eventsThisFrame;
    {
      ...
      {
        mGestures[ eventsThisFrame ] = mGestures[ mReadPosition++ ];
        mReadPosition %= PAN_GESTURE_HISTORY;
      }
    }
    ...
    {
      // Copy the gesture first
      lastReadGesture = mGestures[ readPosition ];
      //if( mProfiling )
      //{
      ...
      if( lastReadGesture.state == Gesture::Started )
      {
        ...
        mPanHistory.clear();
        // NOTE: not used mPredictionHistory.clear();
      }
      else
      {
    ...
    mSmoothingMode = SMOOTHING_LAST_VALUE;//SMOOTHING_MULTITAP;
    mSmoothingAmount = 0.5f;

    mPredictionAmount = 80;//todor use?
    ...
    if( mGestureState != IDLE || justStarted )
    {
      ...
      for( PanInfoHistoryIter it = historyBegin; it != mPanHistory.end(); )
      {
        ...
        else
        {
          ...
          if( lastIt != mPanHistory.end() )
          {
            ...
            //float timeDifference( it->time - lastIt->time );
      ...
      if( tapsUsed > 0 )
      {
        if( divisor > 0.0f )//todor epsilon
        {
          ...
          if( accelerationDivisor > 0.0f )
          {
            ...
            float accelerationSmoothing = 0.15f;//todorw 0.3f
      ...
      const float maximumSecondDerivativeAcceleration = 5.0f;
      ...
      Vector2 confidenceLocal( ( maximumSecondDerivativeAccelerationVector - secondDerivativeAccelerationLocal ) / maximumSecondDerivativeAcceleration );
      Vector2 confidenceScreen( ( maximumSecondDerivativeAccelerationVector - secondDerivativeAccelerationScreen ) / maximumSecondDerivativeAcceleration );
      ...
      mSmoothingAmount = 0.25f;//todorw 0.25f
...
public:
  ...
  {
    const TouchPoint& TEST_point = TEST_touch.points[0];
    ...
    const TouchPoint& TEST_lastPoint = mLastTouch.points[0];
    ...
    int TEST_eventsToProvide = 1;
    if( mFrameNumber % 1 )
    {
      TEST_eventsToProvide = 2;
    }
    mFrameNumber++;
#endif
    ...
    if( TEST_GesturesWaiting >= TEST_eventsToProvide )
    {
    ...
    // Draw raw input.
    DrawLine( mLastPoint.x, mLastPoint.y, TEST_drawToRaw.x, TEST_drawToRaw.y, Vector4( 1.0f, 1.0f, 0.0f, 1.0f ) );
