...
*/
 ...
 // --------Border renderer
    ...
    actor4.SetSize(size);
    actor4.SetParentOrigin( ParentOrigin::BOTTOM_CENTER );
    actor4.SetAnchorPoint( AnchorPoint::TOP_CENTER );
    actor4.SetZ(1.f);
    actor1.Add( actor4 );

    Property::Map propertyMap4;
    propertyMap4.Insert("renderer-type", "border-renderer");
    propertyMap4.Insert("border-color", Color::YELLOW);
    propertyMap4.Insert("border-size", 5.f);

    Toolkit::ControlRenderer borderRenderer = rendererFactory.GetControlRenderer(propertyMap4);
    borderRenderer.SetSize( size );
    borderRenderer.SetOnStage( actor4 );
    borderRenderer.SetDepthIndex( 1.f );
