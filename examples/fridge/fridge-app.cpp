...
const char* BACKGROUND_IMAGE( DEMO_IMAGE_DIR "background.png" );
...
void FridgeApp::OnInitialize( Application& application )
{
  ...
  std::ifstream t(DEMO_SCRIPT_DIR "fridge.json");
  std::string script = std::string( (std::istreambuf_iterator< char >( t )), std::istreambuf_iterator< char >() );
  mBuilder = Builder::New();

  // Set default constants
  mBuilder.AddConstant( "IMAGE_PATH", DEMO_IMAGE_DIR );
  mBuilder.AddConstant( "SCRIPT_PATH", DEMO_IMAGE_DIR );
...
void FridgeApp::CreatePosters()
{
  ...
  for(int i = 0; i < 6; i++)
  {
    ...
    ImageView item = ImageView::New( DEMO_IMAGE_DIR "item0.png" );
    countStr << mPosters.GetItemCount();
    item.SetName("poster-item-" + countStr.str());
    countStr.str(std::string());
    mPosters.AddItem(item);

    item = ImageView::New( DEMO_IMAGE_DIR "item1.png" );
    countStr << mPosters.GetItemCount();
    item.SetName("poster-item-" + countStr.str());
    countStr.str(std::string());
    mPosters.AddItem(item);

    item = ImageView::New( DEMO_IMAGE_DIR "item2.png" );
    countStr << mPosters.GetItemCount();
    item.SetName("poster-item-" + countStr.str());
    countStr.str(std::string());
    mPosters.AddItem(item);

    item = ImageView::New( DEMO_IMAGE_DIR "item3.png" );
    countStr << mPosters.GetItemCount();
    item.SetName("poster-item-" + countStr.str());
    countStr.str(std::string());
    mPosters.AddItem(item);

    item = ImageView::New( DEMO_IMAGE_DIR "item4.png" );
    countStr << mPosters.GetItemCount();
    item.SetName("poster-item-" + countStr.str());
    countStr.str(std::string());
    mPosters.AddItem(item);

    item = ImageView::New( DEMO_IMAGE_DIR "item5.png" );
    countStr << mPosters.GetItemCount();
    item.SetName("poster-item-" + countStr.str());
    countStr.str(std::string());
    mPosters.AddItem(item);
...
void FridgeApp::CreateTopPanel()
{
  ...
  mTopPanel = ImageView::New( DEMO_IMAGE_DIR "banner.png" );
...
void FridgeApp::CreateMenu()
{
  ...
  for(int i = 0; i < 20; i++)
  {
    ...
    countStr << mMenu.GetItemCount();
    menuItem.SetName("menu-item-" + countStr.str());
    countStr.str(std::string());
...
void FridgeApp::CreateMenuBackground()
{
  ...
  mMenuBackground = ImageView::New( DEMO_IMAGE_DIR "white-pixel.png" );
  ...
  mArrowButton = ImageView::New( DEMO_IMAGE_DIR "arrow.png" );
...
void FridgeApp::OnKeyboardEvent(const KeyEvent& event)
{
  if( event.state == KeyEvent::Down )
  {
    ...
    if ( event.keyPressedName == "BackSpace" || event.keyPressedName == "XF86Back" || event.keyPressedName == "Escape")
    {
      if( mIsMenuVisible )
      {
        HideMenu();
    ...
    else if ( event.keyPressedName == "XF86AudioForward" || event.keyPressedName == "XF86LowerChannel" || event.keyPressedName == "Next" )
    {
      if( mIsFocusInPosters )
      {
    ...
    else if ( event.keyPressedName == "XF86AudioRewind" || event.keyPressedName == "XF86RaiseChannel"  || event.keyPressedName == "Prior")
    {
      if( mIsFocusInPosters )
      {
