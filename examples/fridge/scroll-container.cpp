  ...
  switch( panGesture.state )
  {
    ...
    case Gesture::Cancelled:
    {
      ...
      if (absScrollDistance > 1.0f && scrollSpeed > 0.05f) // Threshold TBD
      {
        ...
        if(mScrollDisplacement > 0.0f) // scroll constant distance in constant speed.
        {
          Scroll(stageSize.x * 0.5f, 0.5f, 0.05f, GetFirstVisibleItemId());
        }
        else
        {
          Scroll(-stageSize.x * 0.5f, 0.5f, 0.05f, GetLastVisibleItemId());
        }
      }
      break;
    }
...
{
  if (event.points[0].state == TouchPoint::Motion)
  {
    for( unsigned int i = 0; i < mItemList.size(); ++i )
    {
      if( actor.GetName() == mItemList[i].GetName() )
      {
        mIsFocused = true;
