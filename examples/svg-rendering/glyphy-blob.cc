...
#define SNAP(P) (Point (DEQUANTIZE_X (QUANTIZE_X ((P).x)), DEQUANTIZE_Y (QUANTIZE_Y ((P).y))))
      ...
      for (unsigned i = 0; i < near_endpoints.size (); i++)
      {
        ...
        glyphy_rgba_t encodedArcEndpoint = arc_endpoint_encode (QUANTIZE_X(endpoint.p.x), QUANTIZE_Y(endpoint.p.y), endpoint.d);
        blob.push_back (encodedArcEndpoint.r);
        blob.push_back (encodedArcEndpoint.g);
        blob.push_back (encodedArcEndpoint.b);
        blob.push_back (encodedArcEndpoint.a);
      }

      unsigned int current_endpoints = blob.size ()/4u - offset;

      if (current_endpoints)
      {
        // See if we can fulfill this cell by using already-encoded arcs
        const unsigned char *needle = &blob[offset*4];
        unsigned int needle_len = current_endpoints;
        const unsigned char *haystack = &blob[header_length*4];
        ...
        while (haystack_len >= needle_len) {
          ...
          if (0 == memcmp (1 + (const unsigned char *) needle,
              1 + (const unsigned char *) haystack,
              needle_len * sizeof (*needle)*4 - 1)) {
            found = true;
            break;
          }
          haystack++;
        ...
        if (found) {
          unsigned int new_offset = (haystack - &blob[0])/4;
      ...
      glyphy_rgba_t encodedArcEndpoint = arc_list_encode (offset, current_endpoints, pivots[row*grid_w+col], counts[row*grid_w+col] );
      unsigned int idx = row * grid_w *4 + col*4;
      blob[idx] =  encodedArcEndpoint.r;
      blob[idx+1] =  encodedArcEndpoint.g;
      blob[idx+2] =  encodedArcEndpoint.b;
      blob[idx+3] =  encodedArcEndpoint.a;
      offset = blob.size () / 4;
