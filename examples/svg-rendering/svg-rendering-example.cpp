...
const char* SVG_IMAGES[] =
{
  ...
  DALI_IMAGE_DIR "spring.svg",
  DALI_IMAGE_DIR "spring_bicicleta.svg",
...
public:
  ...
  {
    // Load
    SvgAtlas* encodedAtlas = new SvgAtlas( 1024,512, TEXTURE_ITEM_WIDTH, 8 );
    svg_buffer_t* svgBuffer = svg_buffer_create ();
    glyphy_point_t top_left = {0, 0};
    svg_buffer_move_to (svgBuffer, &top_left);
    svg_encoder_t*  svgEncoder = svg_encoder_create( encodedAtlas );
    ...
    for (NSVGshape* shape = image->shapes; shape != NULL; shape = shape->next)
    {
      svg_buffer_add_svg_shape(svgBuffer, shape, svgEncoder);
      ...
      if( svgBuffer->vertices->size() == (shapeIdx+1)*4 )
      {
        for( unsigned int i = 0; i < 4u; i++ )
        {
          unsigned int idx = shapeIdx*4u + i;
          vertexData->push_back(SVGActor::Vertex( Vector2( (svgBuffer->vertices)->at(idx).x, (svgBuffer->vertices)->at(idx).y ),
              Vector2( (svgBuffer->vertices)->at(idx).g16hi, (svgBuffer->vertices)->at(idx).g16lo ),
    ...
    glyphy_extents_t extents = svgBuffer->ink_extents;
    ...
    nsvgDelete(image);
    svg_buffer_destroy(svgBuffer);
    svg_encoder_destroy(svgEncoder);
