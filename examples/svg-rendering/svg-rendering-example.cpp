...
public:
  ...
  Actor CreateMeshActorFromSVG( const char* svgFile )
  {
    ...
    if( gradientCount > 0 )
    {
      std::vector<unsigned char> gradient_tex_data;
      gradient_tex_data.resize( gradientCount* TEXTURE_ITEM_WIDTH*4);
      generate_gradient_texture(gradient_tex_data, gradientCount, nstops,stops );

      unsigned int tex_pos_x, tex_pos_y;
      encodedAtlas->AtlasAlloc( gradient_tex_data, &tex_pos_x, &tex_pos_y, false );
  ...
  void generate_gradient_texture( std::vector<unsigned char>& gradient_tex,
      int numGradient,
      ...
      const Dali::Vector< NSVGgradientStop*>& stops )
  {
    ...
    for( int n = 0; n < numGradient; n++ )
    {
      ...
      if( curStops[0].offset > 0.f )
      {
        ...
        {
          int index = offset + j;
          gradient_tex[index*4] = color.r;
          gradient_tex[index*4+1] = color.g;
          gradient_tex[index*4+2] = color.b;
          gradient_tex[index*4+3] = color.a;
        }
      }
      ...
      for( int i = 0; i<numStops-1; i++ )
      {
        ...
        for( int j = start; j< end; j++)
        {
          ...
          int index = offset + j;
          gradient_tex[index*4] = currentColor.r;
          gradient_tex[index*4+1] = currentColor.g;
          gradient_tex[index*4+2] = currentColor.b;
          gradient_tex[index*4+3] = currentColor.a;
        }
      }
    }
  }
