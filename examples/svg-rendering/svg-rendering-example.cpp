...
public:
  ...
  Actor CreateMeshActorFromSVG( const char* svgFile )
  {
    ...
    for (NSVGshape* shape = image->shapes; shape != NULL; shape = shape->next)
    {
      svg_encoder_add_svg_shape(svgEncoder, shape);
      ...
      if( svgEncoder->vertices->size() == (shapeIdx+1)*4 )
      {
        for( unsigned int i = 0; i < 4u; i++ )
        {
          unsigned int idx = shapeIdx*4u + i;
          vertexData->push_back(SVGActor::Vertex( Vector2( (svgEncoder->vertices)->at(idx).x, (svgEncoder->vertices)->at(idx).y ),
              Vector2( (svgEncoder->vertices)->at(idx).g16hi, (svgEncoder->vertices)->at(idx).g16lo ),
    ...
    glyphy_extents_t extents = svgEncoder->ink_extents;
