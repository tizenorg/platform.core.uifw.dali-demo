...
int winding_a_cell( int row, int col, double pivot_x, double pivot_y,
                    ...
                    double cell_height)
{
  ...
  double y = extents->min_y + (row+pivot_y) * cell_height;
  for( unsigned int i=0; i<num_endpoints; i++ )
  {
    ...
    Point current(extents->min_x+(col+pivot_x)*cell_width,  y);
    ...
    if( iszero (arc.d) ) // line
    {
      ...
      if( found )
      {
        float location = (x-extents->min_x) / cell_width  + 1.0-pivot_x;
        if(col < int(location) )
        {
          count++;
        }
    ...
    else // arc
    {
      ...
      if( found1 )
      {
        float location = (x1-extents->min_x) / cell_width  + 1.0-pivot_x;
        if(col < int(location) )
        {
          count++;
        }
      ...
      if( found2 )
      {
        float location = (x2-extents->min_x) / cell_width  + 1.0-pivot_x;
        if(col < int(location) )
        {
          count++;
        }
            ...
            int* counts, int* pivots)
{
  ...
  {
    Point p0(0.0,0.0);
    double y = extents->min_y + (row+0.3) * cell_height;
    for( unsigned int i=0; i<num_endpoints; i++ )
    {
      const glyphy_arc_endpoint_t &endpoint = endpoints[i];
      if (endpoint.d == GLYPHY_INFINITY) {
      ...
      if( iszero (arc.d) ) // line
      {
        ...
        if( found )
        {
          float location = (x-extents->min_x) / cell_width  + 0.7;
      ...
      else // arc
      {
        ...
        if( found1 )
        {
          float location = (x1-extents->min_x) / cell_width  + 0.7;
          int divide = int(location);
          if( fabs( location - round( location ) ) < threshold  ){
            pivots[ row*grid_w + int(round( location ))-1 ] = 1;
          }
        ...
        if( found2 )
        {
          float location = (x2-extents->min_x) / cell_width  + 0.7;
          int divide = int(location);
          if( fabs( location - round( location ) ) < threshold  ){
            pivots[ row*grid_w + int(round( location ))-1 ] = 1;
          }
  ...
  for( int row = 0; row<grid_h; row++ )
    for( int col = 0; col<grid_w; col++ )
    {
      ...
      if( pivots[idx] )
      {
        int count = winding_a_cell( row, col, 0.3, 0.7, endpoints, num_endpoints, extents, cell_width, cell_height);
        if( count >= 0 )
        {
          counts[idx] = count;
        }
        ...
        {
          count = winding_a_cell( row, col, 0.7, 0.3, endpoints, num_endpoints, extents, cell_width, cell_height);
          if( count > 0 )
          {
            pivots[idx] = 2;
            counts[idx] = count;
          ...
          {
            count = winding_a_cell( row, col, 0.7, 0.7, endpoints, num_endpoints, extents, cell_width, cell_height);
            pivots[idx] = 3;
            if( count > 0 )
            {
              counts[idx] = count;
            ...
            {
              counts[idx] = 0;
              std::cout<<"fail to get a good pivot!"<<std::endl;
            }
          }
        }
      }
