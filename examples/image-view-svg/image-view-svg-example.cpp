...
public:
  ...
  void Create( Application& application )
  {
    ...
    mSvgActor.SetParentOrigin( ParentOrigin::CENTER_LEFT );
    mSvgActor.SetAnchorPoint( AnchorPoint::CENTER_LEFT );
    stage.Add( mSvgActor );*/

    /*Property::Map propertyMap;
    propertyMap.Insert("rendererType", "svg");
    propertyMap.Insert("imageUrl", SVG_IMAGES[mIndex] );
    mSvgRenderer = rendererFactory.GetControlRenderer( propertyMap);*/
   /* mSvgRenderer = rendererFactory.GetControlRenderer( SVG_IMAGES[mIndex]);
    mSvgRenderer.SetSize( mActorSize );
    mSvgRenderer.SetOnStage( mSvgActor );*/
   ...
   /* Animation animation = Animation::New( 10.f );
    animation.AnimateTo(Property(mSvgActor, Actor::Property::COLOR), Vector4(1.0f, 1.0f, 1.0f, 0.0f), AlphaFunction::BOUNCE, TimePeriod( 2.f, 8.f ));
  ...
  bool OnButtonClicked( Toolkit::Button button )
  {
    ...
    //Toolkit::RendererFactory rendererFactory = Toolkit::RendererFactory::Get();
    //rendererFactory.ResetRenderer( mSvgRenderer, mSvgActor, SVG_IMAGES[mIndex] );

    /*Property::Map propertyMap;
    propertyMap.Insert("rendererType", "svg");
    propertyMap.Insert("imageUrl", SVG_IMAGES[mIndex] );
    rendererFactory.ResetRenderer( mSvgRenderer, mSvgActor, propertyMap );*/
