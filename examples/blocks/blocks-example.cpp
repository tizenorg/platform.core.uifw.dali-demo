...
private:
   ...
   */
  bool OnTouchPaddle(Actor actor, TouchData event)
  {
    if(event.GetPointCount()>0)
    {
      if( event.GetState( 0 ) == PointState::DOWN ) // Commence dragging
      {
        ...
        mRelativeDragPoint = event.GetScreen( 0 );
   ...
   * @param[in] event The touch event
   */
  bool OnTouchLayer(Actor actor, TouchData event)
  {
    if(event.GetPointCount()>0)
    {
      if(mDragActor)
      {
        Vector3 position( event.GetScreen( 0 ) );
