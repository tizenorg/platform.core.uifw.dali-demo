...
public:
  ...
  void Create( Application& application )
  {
    ...
    //actor.SetScale(1.0f);
    actor.SetOrientation(Degree(180.f), Vector3::ZAXIS);
    ...
    float latitude = x_coordinate_billboard[155-30] * 3.14f / 180;
    float longitude = y_coordinate_billboard[155-30] * 3.14f / 180;

    Vector3 location;
    location.x = (float)cos(latitude) * (float)cos(longitude) * (mRadius + 0);
    location.y = (float)sin(longitude) * (mRadius + 0);
    location.z = (float)sin(latitude) * (float)cos(longitude) * (mRadius + 0);
    ...
    cityActor.SetPosition(location);
    actor.Add(cityActor);
