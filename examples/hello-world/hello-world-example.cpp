...
const char* GLOBE_IMAGE(DALI_IMAGE_DIR "worldmap_1024.png");
const char* GLOBE_GRAY_IMAGE(DALI_IMAGE_DIR "worldmap-grayscale.png");
  ...
  const char* FRAGMENT_SHADER = DALI_COMPOSE_SHADER(
    ...
    {                                                                \n
      ...
      mediump vec3 stepLower = step(0.5, dayColorLower.rgb);              \n
                                                                          \n
      //Normalization (values are in the range [0,7])
      //float upper = (dayColorUpper.r*1.0 + dayColorUpper.g*1.0 + dayColorUpper.b) / 3.0;  \n
      //float lower = (dayColorLower.r*1.0 + dayColorLower.g*1.0 + dayColorLower.b) / 3.0;  \n
      //gl_FragColor = vec4(upper, 0.0, 0.0, lower);    \n
      gl_FragColor = vec4(stepUpper * 0.5 + stepLower * 0.5, 1.0);  \n
    }
  );
}
...
public:
  ...
  FrameBufferImage CreateWorldMapImage()
  {
    ...
    ResourceImage image = ResourceImage::New( GLOBE_IMAGE );
    ...
    FrameBufferImage frameBufferImage = FrameBufferImage::New( image.GetWidth(), image.GetHeight()* 0.5f, Pixel::RGBA4444 /*Pixel::LA88*/, RenderBuffer::COLOR );
  ...
  Dali::ShaderEffect NewEffect()
  {
    std::string vertexShader = DALI_COMPOSE_SHADER(
      ...
      void main()
      {
        ...
        vTextureCoords = vec2(aTexCoord.x, aTexCoord.y);
        vTextureCoords0 = vec2(aTexCoord.x, aTexCoord.y * 0.5);     \n
        vTextureCoords1 = vec2(vTextureCoords0.x, vTextureCoords0.y + 0.5);           \n
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      void main()                                                                             \n
      {                                                                                       \n
        mediump vec4 grayColor = texture2D(sTexture, vTextureCoords);                         \n
        grayColor.rgb = step(0.5, grayColor.rgb);                                             \n
                                                                                              \n
        mediump vec4 dayColorUpper = texture2D(sEffect, vTextureCoords0);                     \n
        mediump vec4 dayColorLower = texture2D(sEffect, vTextureCoords1);                     \n
        mediump vec3 stepUpper = step(0.5, dayColorUpper.rgb);                                \n
        mediump vec3 stepLower = step(0.5, dayColorLower.rgb);                                \n
                                                                                              \n
        ...
        // flag : zero in timezone area
        float flag = clamp(dayColorUpperQuantized.r+dayColorUpperQuantized.g+dayColorUpperQuantized.b+dayColorLowerQuantized.r+dayColorLowerQuantized.g+dayColorLowerQuantized.b, 0.0, 1.0);
        float continents = grayColor.r;                                                       \n                                                                                    \n
                                                                                              \n
        mediump vec4 tempColor = flag*continents                                              \n
                               + (1.0-flag)*(vec4(0.965, 0.882, 0.557, 1.0));                 \n
                                                                                              \n
        mediump vec4 dayColor = mix(vec4(0.498, 0.765, 0.894, 1.0), tempColor, continents);   \n

        gl_FragColor = vec4(dayColor.rgb, 1.0);                                               \n
      }                                                                                       \n
    );
    ...
    shaderEffect.SetEffectImage(mWorldmapImage);
#else
    //ResourceImage::New(GLOBE_IMAGE);
    Image image = ResourceImage::New(GLOBE_IMAGE);
  ...
  void Create( Application& application )
  {
    ...
    mGlobeActor = ImageActor::New(ResourceImage::New(GLOBE_GRAY_IMAGE));
    mGlobeActor.SetParentOrigin(ParentOrigin::CENTER);
    mGlobeActor.SetAnchorPoint(AnchorPoint::CENTER);
    mGlobeActor.SetSize(2048.f, 1024.f);
    mGlobeActor.SetScale(6.f);
    ...
    mEffect = NewEffect();
    mCurrentTimeZone = 14;
    mEffect.SetUniform(LIGHT_DIRECTION_PROPERTY_NAME, lightDirection);
    mEffect.SetUniform(TIMEZONE_COLOR_UPPER_PROPERTY_NAME, timezoneSelColorPart1[mCurrentTimeZone]);
    mEffect.SetUniform(TIMEZONE_COLOR_LOWER_PROPERTY_NAME, timezoneSelColorPart2[mCurrentTimeZone]);
    mGlobeActor.SetShaderEffect(mEffect);

    // Respond to a click anywhere on the stage
    stage.GetRootLayer().TouchedSignal().Connect( this, &HelloWorldController::OnTouch );

#if 0
    mTimer = Timer::New(2000.f);
  ...
  bool OnTouch( Actor actor, const TouchEvent& touch )
  {
    ...
    {
        mTouchedDownPos = touch.GetPoint(0).screen;
    }
    else
    {
      Vector2 diff = mTouchedDownPos - touch.GetPoint(0).screen;
      if (mGlobeActor)
      {
          mCurrentPos -= (diff * 0.1f);
          //mCurrentPos.x -= (diff.x * 0.1f);
          mGlobeActor.SetPosition(mCurrentPos.x, mCurrentPos.y);
      }
    }
...
private:
  ...
  ImageActor       mGlobeActor;

  ShaderEffect     mEffect;

  Vector2          mTouchedDownPos;
  Vector2          mCurrentPos;
