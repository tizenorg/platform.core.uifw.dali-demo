...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    std::string vertexShader = DALI_COMPOSE_SHADER(
      ...
      void main()\n
      {
        ...
        vBrightness = 1.0 - dot(uLightDirection, normal);
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      void main()
      {
        ...
        float nightColor = 0.5 * 0.15;
        //nightColor = clamp(nightColor, 0.0, 1.0);

        gl_FragColor = vec4(dayColor.rgb*(0.8 + 0.2*nightColor) - vBrightness * nightColor * 4.0, 1.0);
      }
    );
  ...
  void Create( Application& application )
  {
    ...
    lightDirection *= Quaternion(Degree(90.f), Vector3::YAXIS);
    ...
    globeActor.SetScale(1.5f, 1.5f, 1.5f);

    Animation anim = Animation::New(6.f);
    anim.SetLooping(true);
#if 0
    anim.AnimateBy(Property(globeActor, Actor::Property::ORIENTATION), Quaternion(Radian(Degree(360.f)), Vector3::YAXIS), AlphaFunction::LINEAR);
#else
