...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      void main()
      {
        ...
        float flag = clamp(0.0/*inRangeForTZHighlight*/ + dayColorUpperQuantized.r+dayColorUpperQuantized.g+dayColorUpperQuantized.b+dayColorLowerQuantized.r+dayColorLowerQuantized.g+dayColorLowerQuantized.b, 0.0, 1.0);
  ...
  void Create( Application& application )
  {
     ...
     * Globe model
     */
    MeshActor globeActor = MeshActor::New(CreateMesh());
    globeActor.SetName("mesh actor");
    globeActor.SetParentOrigin(ParentOrigin::CENTER);
    globeActor.SetAnchorPoint(AnchorPoint::CENTER);
    globeActor.RotateBy(Quaternion(Degree(90.f), Vector3::YAXIS));
    globeActor.RotateBy(Quaternion(Degree(180.f), Vector3::ZAXIS));
    stage.Add(globeActor);
    ...
    globeActor.SetShaderEffect(effect);
    ...
    printf("latitude : %.2f, longitude : %.2f \n", latitude / 3.14f * 180.f, longitude / 3.14f * 180.f);
    ...
    globeActor.Add(cityActor);

#if 0
    /**
     * Base city
     */
    Vector3 baseCityLocation;
    baseCityLocation.x = (float)cos(0.f) * (float)cos(0.f) * (mRadius + 0);
    baseCityLocation.y = (float)sin(0.f) * (mRadius + 0);
    baseCityLocation.z = (float)sin(0.f) * (float)cos(0.f) * (mRadius + 0);

    Actor baseCityActor = CreateSolidColorActor(Color::BLUE);
    baseCityActor.SetParentOrigin(ParentOrigin::CENTER);
    baseCityActor.SetAnchorPoint(AnchorPoint::CENTER);
    baseCityActor.SetSize(10.f, 10.f);
    baseCityActor.SetPosition(baseCityLocation);
    baseCityActor.RotateBy(Degree(90.f), Vector3::YAXIS);
    globeActor.Add(baseCityActor);
#endif

    worldXAxis *= Quaternion(Radian(3.1415f * 0.5f + latitude), Vector3::YAXIS);
    worldXAxis *= Quaternion(Degree(180.f), Vector3::ZAXIS);

    Animation anim = Animation::New(5.f);
    anim.SetLooping(true);
    anim.AnimateBy(Property(globeActor, Actor::Property::ORIENTATION), Quaternion(Radian(latitude), Vector3::YAXIS), AlphaFunction::BOUNCE);
    anim.AnimateBy(Property(globeActor, Actor::Property::ORIENTATION), Quaternion(Radian(longitude), worldXAxis), AlphaFunction::BOUNCE);
    anim.AnimateBy(Property(globeActor, Actor::Property::SCALE), Vector3(2.5f, 2.5f, 2.5f), AlphaFunction::BOUNCE);
    anim.Play();
