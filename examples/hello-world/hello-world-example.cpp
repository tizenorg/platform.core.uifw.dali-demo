  ...
  // Get a handle to the stage
    ...
    for (int index = 0; index < 367 - 37; index++)
    {
      if (ZOOM_LEVEL[index] == 1)
      {
        ...
        cityLocation.x = (float)cos(latitude) * (float)cos(longitude) * (mRadius * 1.5f);
        cityLocation.y = (float)sin(longitude) * (mRadius * 1.5f);
        cityLocation.z = (float)sin(latitude) * (float)cos(longitude) * (mRadius * 1.5f);
        ...
        cityActor.SetSize(0.75f * 0.25, 0.75f * 0.25f);
