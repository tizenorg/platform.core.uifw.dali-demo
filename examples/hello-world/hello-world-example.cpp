...
const char* GLOBE_IMAGE(DALI_IMAGE_DIR "worldmap.png");
const char* GLOBE_RGB_IMAGE(DALI_IMAGE_DIR "worldmap_rgb.bmp");

const std::string LIGHT_DIRECTION_PROPERTY_NAME("uLightDirection");
const std::string TIMEZONE_COLOR_PROPERTY_NAME("uTimezoneColor");
...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      {                                                                                       \n                                                                                            \n
        ...
        //mediump vec3 stepColor = step(0.4, textureColor.rgb);                                 \n
        mediump vec3 stepColor = textureColor.rgb;
                                                                                              \n
        mediump vec3 colorQuantized = abs(textureColor.rgb - uTimezoneColor);                 \n
                                                                                              \n
        // flag : zero in timezone area
        float flag = clamp(length(colorQuantized), 0.0, 1.0); \n
        //flag = flag * 2.0 ;
        flag = step(0.4, flag);                                                             \n
                                                                                              \n
        float continents = length(stepColor);                                                 \n
        continents = step(0.6, continents);
                                                                                              \n
        //gl_FragColor = vec4(flag, flag,flag, 1.0); return ;                                            \n
  ...
  void Create( Application& application )
  {
    ...
    mGlobeActor = ImageActor::New(ResourceImage::New(GLOBE_RGB_IMAGE));
    ...
    mEffect.SetUniform(TIMEZONE_COLOR_PROPERTY_NAME, timezoneSelColorPart1[mCurrentTimeZone] * 0.5f + timezoneSelColorPart2[mCurrentTimeZone] * 0.5f);
