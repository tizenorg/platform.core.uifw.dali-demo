...
void main()
{
  vTexCoord0 = vec2( aTexCoord.x, (1.0 - aTexCoord.y) * 0.5);
  vTexCoord1 = vec2(vTexCoord0.x, vTexCoord0.y + 0.5);
...
void main()
{
  mediump vec4 dayColorUpper = texture2D(sTexture, vTexCoord0);
  mediump vec4 dayColorLower = texture2D(sTexture, vTexCoord1);
  mediump vec3 stepUpper = step(0.5, dayColorUpper.rgb);
  mediump vec3 stepLower = step(0.5, dayColorLower.rgb);

  //Normalization (values are in the range [0,3])
  gl_FragColor = vec4( (stepUpper.x+stepUpper.y+stepUpper.z)/3.0, (stepLower.x+stepLower.y+stepLower.z)/3.0, 0.0, 1.0 );
}
);
}
...
struct CityOpacityConstraint
{
  CityOpacityConstraint()
  {
  }

  void operator()(float& current, const PropertyInputContainer& inputs)
  {
    float zPos = inputs[0]->GetFloat();
    if(zPos < 50.f)
    {
      current = 0.f;
    }
    else if(zPos >= 50.f && zPos < 100.f)
    {
      current = (zPos - 50.f) / 50.f;
    }
    else
    {
      current = 1.f;
...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    std::string vertexShader = DALI_COMPOSE_SHADER(
      ...
      void main()
      {
        ...
        vTextureCoords = vec2(1.0 - aTexCoord.x, aTexCoord.y / 2.0);
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      uniform float  uTimezoneColorUpper;
      uniform float  uTimezoneColorLower;
      ...
      void main()
      {
        ...
        mediump vec4 textureColor = texture2D(sTexture, vTextureCoords);

        mediump float stepUpper = textureColor.x * 3.0;
        mediump float stepLower = textureColor.y * 3.0;

        mediump float dayColorUpperQuantized = abs(stepUpper - uTimezoneColorUpper);
        mediump float dayColorLowerQuantized = abs(stepLower - uTimezoneColorLower);
        float flag = clamp(dayColorUpperQuantized+dayColorLowerQuantized, 0.0, 1.0);

        float continents = clamp(stepUpper*stepLower, 0.0, 1.0);
  ...
  // Create vertices and specify their color
    ...
    mWorldmapImage = CreateWorldMapImage();
  ...
  // Get a handle to the stage
    ...
    effect.SetUniform(TIMEZONE_COLOR_UPPER_PROPERTY_NAME, timezoneSelColorPart1[mCurrentTimeZone].x+timezoneSelColorPart1[mCurrentTimeZone].y+timezoneSelColorPart1[mCurrentTimeZone].z);
    effect.SetUniform(TIMEZONE_COLOR_LOWER_PROPERTY_NAME, timezoneSelColorPart2[mCurrentTimeZone].x+timezoneSelColorPart2[mCurrentTimeZone].y+timezoneSelColorPart2[mCurrentTimeZone].z);
    mGlobeActor.SetShaderEffect(effect);

    /**
     * City
     */
    mCityLayer = Layer::New();
    mCityLayer.SetParentOrigin(ParentOrigin::CENTER);
    mCityLayer.SetAnchorPoint(AnchorPoint::CENTER);
    mCityLayer.RaiseToTop();
    stage.Add(mCityLayer);

    mCityLayerRotateConstraint = Constraint::New<Quaternion>(mCityLayer, Actor::Property::ORIENTATION, EqualToConstraint() );
    mCityLayerRotateConstraint.AddSource(Source(mGlobeActor, Actor::Property::ORIENTATION));
    mCityLayerRotateConstraint.Apply();

    //int index = 62-37;  // New york
    //int index = 58-37;  // Lima
    //int index = 162-37;   // Seoul
    for (int index = 0; index < 367 - 37; index++)
    {
      //if (ZOOM_LEVEL[index] == 1)
      {
        ...
        cityLocation.x = (float)cos(latitude) * (float)cos(longitude) * (mRadius * 1.0f);
        cityLocation.y = (float)sin(longitude) * (mRadius * 1.0f);
        cityLocation.z = (float)sin(latitude) * (float)cos(longitude) * (mRadius * 1.0f);

#if 0
        TextLabel cityActor = TextLabel::New(CITIES[index]);
#else
        Vector4 colors[] =
        {
          Color::RED,
        ...
        Actor cityActor = CreateSolidColorActor(colors[ZOOM_LEVEL[index]-1]);
#endif
        //cityActor.SetResizePolicy(ResizePolicy::USE_NATURAL_SIZE, Dimension::ALL_DIMENSIONS);
        ...
        cityActor.SetSize(10.f, 10.f);//0.75f * 0.25, 0.75f * 0.25f);
        cityActor.SetInheritOrientation(false);
        cityActor.SetInheritScale(false);
        mCityLayer.Add(cityActor);

        Constraint cityAlphaConstraint = Constraint::New<float>(cityActor, Actor::Property::COLOR_ALPHA, CityOpacityConstraint());
        cityAlphaConstraint.AddSource(LocalSource(Actor::Property::WORLD_POSITION_Z));
        cityAlphaConstraint.Apply();
      }
    }
    ...
    mPinchDetector.Attach(eventActor);
    mPinchDetector.DetectedSignal().Connect(this, &HelloWorldController::OnPinch);
#endif
  }
  ...
  void OnPinch(Actor actor, const PinchGesture& gesture)
  {
    switch (gesture.state)
    {
      case Gesture::Started:
        mPinchInitScale = gesture.scale;
        printf("pinch started [%.2f]\n", mPinchInitScale);
        break;

      case Gesture::Continuing:
      {
        mScaled += mPinchInitScale - gesture.scale;

        mScaled = (mScaled > 4.f) ? 4.f : mScaled;
        mScaled = (mScaled < 1.f) ? 1.f : mScaled;

        printf("pinch continued [%.2f]\n", mScaled);

        mGlobeActor.SetScale(mScaled * 0.5f + 0.5f);
        mCityLayer.SetScale(mScaled * 0.5f + 0.5f);
        break;
      }

      case Gesture::Finished:
        printf("pinch finished \n");
        break;
  ...
  FrameBufferImage CreateWorldMapImage()
  {
    Stage stage = Stage::GetCurrent();

    ShaderEffect shader = ShaderEffect::New( VERTEX_SHADER, FRAGMENT_SHADER );
    ResourceImage image = ResourceImage::New( DALI_IMAGE_DIR "worldmap.png" );
    ImageActor actor = ImageActor::New(image);
    actor.SetParentOrigin( ParentOrigin::CENTER );
    actor.SetAnchorPoint( AnchorPoint::CENTER );
    actor.SetPosition( 0.0f,0.0f,0.0f );
    actor.SetSize(image.GetWidth(), image.GetHeight()*0.5f);
    actor.SetShaderEffect(shader);
    stage.Add( actor );

    //Create a camera actor
    CameraActor camera = CameraActor::New(Size( image.GetWidth(), image.GetHeight()*0.5f));
    camera.SetParentOrigin( ParentOrigin::CENTER );
    camera.SetAnchorPoint( AnchorPoint::CENTER );
    camera.SetOrthographicProjection(Size( image.GetWidth(), image.GetHeight()*0.5f));
    stage.Add( camera );

    //Create a framebuffer image
    FrameBufferImage frameBufferImage = FrameBufferImage::New( image.GetWidth(), image.GetHeight()* 0.5f, Pixel::RGB888, RenderBuffer::COLOR );

    //Create an offscreen rendertask
    RenderTaskList  renderTaskList = stage.GetRenderTaskList();
    RenderTask task = renderTaskList.CreateTask();
    task.SetViewport(Viewport(0,0,image.GetWidth(),image.GetHeight()*0.5f));
    task.SetCameraActor(camera);
    task.SetSourceActor( actor );
    task.SetExclusive(true);
    task.SetTargetFrameBuffer(frameBufferImage);
...
private:
  ...
  FrameBufferImage mWorldmapImage;
  Layer         mCityLayer;
  Constraint    mCityLayerRotateConstraint;

  PanGestureDetector   mPanDetector;
  PinchGestureDetector mPinchDetector;
  float                mPinchInitScale;
  float                mScaled;
