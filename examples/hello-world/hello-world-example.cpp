  ...
  // Get a handle to the stage
    ...
    for (int index = 0; index < 367 - 37; index++)
    {
      if (ZOOM_LEVEL[index] == 1)
      {
        ...
        cityLocation.x = (float)cos(latitude) * (float)cos(longitude) * (mRadius);
        cityLocation.y = (float)sin(longitude) * (mRadius);
        cityLocation.z = (float)sin(latitude) * (float)cos(longitude) * (mRadius);

        TextLabel cityActor = TextLabel::New(CITIES[index]);
        cityActor.SetParentOrigin(ParentOrigin::CENTER);
        cityActor.SetAnchorPoint(AnchorPoint::BOTTOM_RIGHT);
        cityActor.SetPosition(cityLocation);
        cityActor.SetSize(0.75f, 0.75f * 0.5f);
        cityActor.SetInheritOrientation(false);
        cityActor.SetInheritScale(false);
        //cityActor.SetSortModifier(-1000);
