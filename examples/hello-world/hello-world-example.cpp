...
const char* GLOBE_IMAGE(DALI_IMAGE_DIR "worldmap_1536.png");
...
public:
  ...
  void Create( Application& application )
  {
    ...
    mGlobeActor = ImageActor::New(ResourceImage::New(GLOBE_IMAGE));
  ...
  bool OnTouch( Actor actor, const TouchEvent& touch )
  {
    if (mGlobeActor)
    {
      ...
      {
        mTouchedDownPosition = touch.GetPoint(0).screen;
      }
      else
      {
        Vector2 touchDelta = mTouchedDownPosition - touch.GetPoint(0).screen;
        mCurrentPosition -= touchDelta;

        mGlobeActor.SetPosition(mCurrentPosition.x, mCurrentPosition.y);
      }
    }
