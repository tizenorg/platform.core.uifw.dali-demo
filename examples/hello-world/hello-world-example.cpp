...
public:
  ...
  void Create(Application& application)
  {
    ...
    stage.GetRootLayer().TouchedSignal().Connect(this, &HelloWorldController::OnTouched);
  }
  ...
  bool OnTouched(Actor actor, const TouchEvent& event)
  {
    ...
    else if (TouchPoint::Motion == touchPoint.state)
    {
      ...
      mDiffAccumulated += diff;

      if (diff.x)
      {
        localRotation = mGlobeRotation * Quaternion(Radian(-diff.x / (Math::PI * 32.f)), Vector3::YAXIS);
        mGlobeRotation = localRotation;
      }

      if (mDiffAccumulated.y > 60.f)
      {
        mDiffAccumulated.y = 61.f;
      }
      else if (mDiffAccumulated.y < -60.f)
      {
        mDiffAccumulated.y = -61.f;
      }
      ...
      localRotation = mGlobeRotation * Quaternion(Radian(-mDiffAccumulated.y / (Math::PI * 32.f)), worldXAxis);
