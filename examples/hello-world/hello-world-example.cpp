  ...
  const char* FRAGMENT_SHADER = DALI_COMPOSE_SHADER(
    ...
    {                                                                \n
      ...
      mediump vec3 stepLower = step(0.5, dayColorLower.rgb);              \n
                                                                          \n
      //Normalization (values are in the range [0,4])
      float upper = (stepUpper.r*4.0 + stepUpper.g*2.0 + stepUpper.b) / 7.0;  \n
      float lower = (stepLower.r*4.0 + stepLower.g*2.0 + stepLower.b) / 7.0;  \n
...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      void main()                                                                           \n
      {                                                                                     \n
        mediump vec4 texColor = texture2D(sTexture, vTextureCoords) * 7.0;                  \n
                                                                                            \n
        mediump vec3 stepUpper = vec3(mod(floor(texColor.r/4.f), 4.f), mod(floor(texColor.r/2.f), 2.f), mod(texColor.r, 2.f)); \n
        mediump vec3 stepLower = vec3(mod(floor(texColor.a/4.f), 4.f), mod(floor(texColor.a/2.f), 2.f), mod(texColor.a, 2.f)); \n
                                                                                            \n
        ...
        gl_FragColor = vec4(dayColor.rgb - vBrightness * nightColor, 1.0);                  \n
      }                                                                                     \n
    );
  ...
  void Create( Application& application )
  {
    ...
    mGlobeActor = ImageActor::New(mWorldmapImage);
