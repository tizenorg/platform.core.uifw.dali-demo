...
const char* VERTEX_SHADER_COMBINE = DALI_COMPOSE_SHADER(
...
const char* FRAGMENT_SHADER_COMBINE = DALI_COMPOSE_SHADER(
varying mediump vec2  vTexCoord0;
varying mediump vec2  vTexCoord1;
...
void main()
{
  ...
  gl_FragColor = vec4( (stepUpper.x+stepUpper.y+stepUpper.z)/255, (stepLower.x+stepLower.y+stepLower.z)/255, 0.0, 1.0 );
}
);
...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      void main()
      {
        ...
        medium float timezoneColorUpper = uTimezoneColorUpper.x + uTimezoneColorUpper.y + uTimezoneColorUpper.z;
        medium float timezoneColorLower = uTimezoneColorLower.x + uTimezoneColorLower.y + uTimezoneColorLower.z;
  ...
  FrameBufferImage CreateWorldMapImage()
  {
    ...
    //Create a renderer
    Shader shader = Shader::New( VERTEX_SHADER_COMBINE, FRAGMENT_SHADER_COMBINE );
    Geometry geometry = CreateGeometry();
    Image image = ResourceImage::New( WORLDMAP_IMAGE );
    Sampler sampler = Sampler::New(image, "sTexture");
    Material material = Material::New( shader );
    material.AddSampler( sampler );
    Renderer renderer = Renderer::New( geometry, material );

    //Create an actor to render the renderer
    Size imageSize( image.GetWidth(), image.GetHeight()*0.5f );
    ...
    CameraActor camera = CameraActor::New(imageSize);
    camera.SetParentOrigin( ParentOrigin::CENTER );
    camera.SetAnchorPoint( AnchorPoint::CENTER );
    camera.SetOrthographicProjection(imageSize);
    stage.Add( camera );

    //Create the resulting framebuffer image
    FrameBufferImage image = FrameBufferImage::New( image.GetWidth(), image.GetHeight()* 0.5f, Pixel::RGB888, RenderBuffer::COLOR );

    //Create an offscreen rendertask
    RenderTaskList  renderTaskList = stage.GetRenderTaskList();
    RenderTask task = renderTaskList.CreateTask();
    task.SetViewport(Viewport(0,0,imageSize.width, imageSize.height));
    task.SetCameraActor(camera);
    task.SetSourceActor( actor );
    task.SetExclusive(true);
    task.SetRefreshRate(RenderTask::REFRESH_ONCE);
    task.SetTargetFrameBuffer(mWorldmapImage);
