...
namespace
{
  ...
  const char* FRAGMENT_SHADER = DALI_COMPOSE_SHADER(
    ...
    {                                                                \n
      ...
      float upper = (dayColorUpper.r*4.0 + dayColorUpper.g*2.0 + dayColorUpper.b) / 7.0;  \n
      float lower = (dayColorLower.r*4.0 + dayColorLower.g*2.0 + dayColorLower.b) / 7.0;  \n
...
public:
  ...
  Dali::ShaderEffect NewEffect()
  {
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      void main()                                                                           \n
      {                                                                                     \n
        mediump vec4 globeTexColor = texture2D(sTexture, vTextureCoords);                   \n
        float globeColor = step(0.5, globeTexColor.r);                                      \n
                                                                                            \n
        mediump vec4 tzTexColor    = texture2D(sEffect,  vTextureCoords) * 7.0;             \n
                                                                                            \n
        mediump vec3 stepUpper = vec3(mod(floor(tzTexColor.r/4.0), 4.0), mod(floor(tzTexColor.r/2.0), 2.0), mod(tzTexColor.r, 2.0)); \n
        mediump vec3 stepLower = vec3(mod(floor(tzTexColor.a/4.0), 4.0), mod(floor(tzTexColor.a/2.0), 2.0), mod(tzTexColor.a, 2.0)); \n
                                                                                            \n
        mediump vec3 dayColorUpperQuantized = step(0.01, abs(stepUpper - uTimezoneColorUpper));         \n
        mediump vec3 dayColorLowerQuantized = step(0.01, abs(stepLower - uTimezoneColorLower));         \n
                                                                                            \n
        ...
        float continents = clamp(globeColor, 0.0, 1.0);                      \n
