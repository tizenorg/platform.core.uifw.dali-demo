...
const char* GLOBE_RGB_IMAGE(DALI_IMAGE_DIR "worldmap_rgb_border.bmp");
...
namespace
{
  ...
  const char* FRAGMENT_SHADER = DALI_COMPOSE_SHADER(
    ...
    {                                                                \n
      ...
      gl_FragColor = dayColorUpper * 0.5 + dayColorLower * 0.5;  \n
    }
  );
}
...
public:
    ...
    std::string fragmentShader = DALI_COMPOSE_SHADER(
      ...
      {                                                                                     \n
        ...
        float globeColor = step(0.5, globeTexColor.r);                                      \n
                                                                                            \n
        mediump vec4 tzTexColor    = texture2D(sTexture,  vTextureCoords);                  \n
                                                                                            \n
        mediump vec3 stepUpper = tzTexColor.rgb;                                            \n                                                                                            \n
        mediump vec3 dayColorUpperQuantized = abs(stepUpper - uTimezoneColorUpper);         \n
                                                                                            \n
        // flag : zero in timezone area
        float flag = clamp(length(dayColorUpperQuantized), 0.0, 1.0);                       \n
        //flag = step(0.5, flag);                                                             \n

        //float continents = clamp((stepUpper.r+stepUpper.g+stepUpper.b)*(stepLower.r+stepLower.g+stepLower.b), 0.0, 1.0);                      \n
        float continents = length(stepUpper);               \n
        //continents = smoothstep(0.0, 1.0, continents * 0.1) * 10.0;
        //continents = step(0.5, continents);   \n
  ...
  void Create( Application& application )
  {
    ...
    effect.SetUniform(TIMEZONE_COLOR_UPPER_PROPERTY_NAME, timezoneSelColorPart1[mCurrentTimeZone] * 0.5f + timezoneSelColorPart2[mCurrentTimeZone] * 0.5f);
