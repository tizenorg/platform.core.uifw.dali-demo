...
public:
  ...
  Cluster CreateClusterActor(ClusterType clusterType, ClusterStyle style)
  {
    ...
    float minStageSize = min(stageSize.x, stageSize.y);
    clusterActor.SetSize(minStageSize * CLUSTER_RELATIVE_SIZE, minStageSize * CLUSTER_RELATIVE_SIZE, 0.0f);
  ...
  void AddCluster(ClusterType clusterType, ClusterStyle style)
  {
    ...
    Vector3 clusterPosition = Vector3(CLUSTER_COLUMN_INDENT * stageSize.width + row * (clusterRightShift * stageSize.width - minStageSize * CLUSTER_RELATIVE_SIZE),
                                      CLUSTER_ROW_INDENT * stageSize.height + row * (clusterRightShift * stageSize.height - minStageSize * CLUSTER_RELATIVE_SIZE), 0.0f);
