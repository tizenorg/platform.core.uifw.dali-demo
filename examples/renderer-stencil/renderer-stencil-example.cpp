...
const float   CUBE_WIDTH_SCALE                   = 0.32f;
// The width and height of the floor object.
const Vector2 FLOOR_DIMENSION_SCALE              = Vector2( 0.67f, 0.017f );
...
const Vector4 REFLECTION_COLOR                   = Vector4( 0.6f, 0.6f, 0.6f, 0.6f ); ///< Note that alpha is not 1.0f, to make the blend more photo-realistic.

// We need to control the draw order as we are controlling both the stencil and depth buffer per renderer.
// This value is the gap in depth-index in-between each renderer.
const int DEPTH_INDEX_GRANULARITY                = 10000;
