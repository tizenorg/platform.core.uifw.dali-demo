...
#define SCALE  (1. * (1 << 0))
  ...
  FT_Set_Char_Size( face,
                    ...
                    96,
                    96 );
  if ( FT_Err_Ok != FT_Load_Glyph (face, glyph_index,FT_LOAD_DEFAULT) )
...
//    die ("FreeType loaded glyph format is not outline");
  ...
  std::cout << "face units_per_EM: " << face->units_per_EM
            ...
            << " horiAdvance:  "     << face->glyph->metrics.horiAdvance << std::endl;
			 ...
			 glyph_info_t *glyph_info)
{
  ...
  if (!glyph_info->is_empty)
	  font->atlas->AtlasAlloc (buffer, output_len,
