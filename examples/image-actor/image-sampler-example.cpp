...
const unsigned int TOOLBAR_HEIGHT = 72;
...
private:
  ...
  Scene         mCurrentScene;           ///< The current scene being viewed
...
: mApplication( application ),
  mCurrentScene( FILTER_MODE_SCENE ),
  mImageScale( 1.0f ),
  mCurrentImage( 0 ),
  mMenuShown( false )
{
...
void ImageActorExample::CreateTextureFilteringScene()
{
  ...
  mImageLayoutTable.SetFixedHeight( 0, TOOLBAR_HEIGHT );
...
{
  if( mCurrentScene != FILTER_MODE_SCENE )
  {
    mCurrentScene = FILTER_MODE_SCENE;
    CreateCurrentScene();
  }
...
{
  if( mCurrentScene != WRAP_MODE_SCENE )
  {
    mCurrentScene = WRAP_MODE_SCENE;
    CreateCurrentScene();
  }
...
void ImageActorExample::CreateCurrentScene()
{
  UnloadView();

  switch( mCurrentScene )
  {
    case FILTER_MODE_SCENE:
    {
      CreateTextureFilteringScene();
      break;
    }
    case WRAP_MODE_SCENE:
    {
