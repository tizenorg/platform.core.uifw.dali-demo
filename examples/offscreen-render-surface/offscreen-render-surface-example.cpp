...
public:
  ...
  , mFinished( false )
  {
    ...
    mNativeRenderSurface = CreateNativeSurface( PositionSize( 0, 0, WIDTH, HEIGHT ), "TestNativeRenderSurface", true );

    Any nativeWindow = ecore_wl_window_new(NULL, 0, 0, WIDTH, HEIGHT, 0);
    mAdaptor = &Adaptor::New( nativeWindow, *mNativeRenderSurface, Configuration::APPLICATION_DOES_NOT_HANDLE_CONTEXT_LOSS );
    ...
    mNativeRenderSurface->SetRenderNotification(mRenderNotification);
  ...
  void OnPostRender()
  {
    ...
    mBuffer->Copy( mNativeRenderSurface->GetDrawable());

    mNativeRenderSurface->ReleaseSurface();
    mFinished = true;
  }
...
private:
  ...
  NativeRenderSurface* mNativeRenderSurface;
  Adaptor* mAdaptor;
