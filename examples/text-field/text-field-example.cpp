...
public:
  ...
  void OnTapGesture( Actor actor, const TapGesture& gesture )
  {
    ...
    {
      std::cout << "TextLabel ay have been touched" << std::endl;
  ...
  void OpenFolderForEdit( Actor& actor )
  {
    for( std::vector< ItemData >::iterator iter = mItems.begin(); iter != mItems.end(); ++iter )
    {
      ...
      std::cout <<  (*iter).container << std::endl;
  ...
  {
    Animation shakeAnimation = Animation::New( 0.4f );

    KeyFrames keyFrames = KeyFrames::New();
    keyFrames.Add(0.0f, AngleAxis(Degree(4), Vector3::ZAXIS));
    keyFrames.Add(0.25f, AngleAxis(Degree(0), Vector3::ZAXIS));
    keyFrames.Add(0.5f, AngleAxis(Degree(-4), Vector3::ZAXIS));
    keyFrames.Add(1.0f, AngleAxis(Degree(-0), Vector3::ZAXIS));

    shakeAnimation.AnimateBetween( Property( mContainer, Actor::Property::ORIENTATION ), keyFrames );
    ...
    if ( !mNotifyingMaxCharactersExceeded )
    {
      ...
      mMaxCharactersExceededMessage.SetProperty( TextLabel::Property::TEXT_COLOR, Color::WHITE );
      mMaxCharactersExceededMessage.SetParentOrigin( ParentOrigin::CENTER );
      mMaxCharactersExceededMessage.SetAnchorPoint( AnchorPoint::CENTER );
      mContainer.Add ( mMaxCharactersExceededMessage );

      Animation enlargeLabelAnimation = Animation::New( 2.0f );
      mMaxCharactersExceededMessage.SetProperty( Actor::Property::SCALE, Vector3( .15f, .15f, 1.0f )  );
      enlargeLabelAnimation.AnimateTo( Property( mMaxCharactersExceededMessage, Actor::Property::SCALE ), Vector3( 1.5f, 1.5f, 1.0f ), AlphaFunction::SIN  );
      enlargeLabelAnimation.FinishedSignal().Connect( this, &TextFieldExample::OnTextEnlargeAnimationFinished );

      enlargeLabelAnimation.Play();
    }
    shakeAnimation.Play();
  }
  ...
  void SetUpTextField( TextField& field, Size stageSize, Actor& parent, std::string& text )
  {
    if ( !field )
    {
      ...
      // field.SetProperty( TextLabel::Property::TEXT_COLOR, Color::WHITE ); todo Seems to get an INTEGER and crashes
