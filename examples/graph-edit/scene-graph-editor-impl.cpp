...
void SceneGraphEditor::DrawCurve( SplineData& splineData, Dali::Vector2 startPosition, Dali::Vector2 endPosition )
{
  ...
  Dali::PropertyBuffer pentagonVertices = Dali::PropertyBuffer::New( vertexFormat, pointCount );
  pentagonVertices.SetData( vertexData );
  delete []vertexData;
  ...
  Dali::PropertyBuffer indices = Dali::PropertyBuffer::New( indexFormat, indexCount );
  ...
  if( !splineData.actor )
  {
    ...
    splineData.material = Dali::Material::New( shader );
    Dali::ResourceImage image = Dali::ResourceImage::New( MATERIAL_SAMPLE );
    splineData.material.AddTexture( image, "sTexture" );
  ...
  else
  {
    ...
    // Remove old renderer.
    splineData.actor.RemoveRenderer( 0 );
  }

  Dali::Renderer renderer = Dali::Renderer::New( geometry, splineData.material );
  splineData.actor.AddRenderer( renderer );
  renderer.SetProperty( Dali::Renderer::Property::DEPTH_INDEX, 0 );
}
