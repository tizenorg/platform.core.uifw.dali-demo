...
void ImageChannelControl::SetImage( const std::string& url )
{
  ...
  properties.Insert( "rendererType", "image" );
  properties.Insert( "url", url );
  Property::Map shader;
  shader.Insert("fragmentShader", FRAGMENT_SHADER);
  properties.Insert("shader", shader);

  Toolkit::RendererFactory::Get().ResetRenderer( mRenderer, self, properties );
  RelayoutRequest();
}
...
Vector3 ImageChannelControl::GetNaturalSize()
{
  ...
  {
    Vector2 naturalSize;
    mRenderer.GetNaturalSize(naturalSize);
