...
public:
  ...
  bool Tick()
  {
    ...
    ImageView imageView = ImageView::New( mNativeImage );
  ...
  void SetPixelBufferIntoTBMsurface( Any& image, std::string filename )
  {
    ...
    unsigned char* rawLineBufferPtr = NULL;
    ...
    if( in.is_open() )
    {
      ...
      rawLineBufferPtr = rawBufferPtr;

      for( int r = 0; r < height; ++r )
      {
        memcpy( bufferPtr, rawLineBufferPtr, lineSize);
        bufferPtr += (r*stride);
