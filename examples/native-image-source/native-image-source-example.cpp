...
namespace
{
  const std::string JPG_FILENAME = DALI_IMAGE_DIR "gallery-medium-1.jpg";
        ...
        case Pixel::RGBA8888:
        {
          ...
          for( unsigned int r = 0; r < height; ++r )
          {
            for( unsigned int c = 0; c < width; ++c )
            {
              ...
              pixRGBBuf[rgbOffset + 0] = pixBuf[rgbaOffset + 0];
              pixRGBBuf[rgbOffset + 1] = pixBuf[rgbaOffset + 1];
              pixRGBBuf[rgbOffset + 2] = pixBuf[rgbaOffset + 2];
            }
          }
