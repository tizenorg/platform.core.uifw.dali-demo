...
void SetLabel( Toolkit::PushButton button, const char* label )
{
...
public:
  ...
  void CreateControls()
  {
...

    {
      ...
      Toolkit::PushButton button = CreateButton( FITTING_BUTTON_ID, StringFromScalingMode( mScalingMode ) );
...

    {
      ...
      Toolkit::PushButton button = CreateButton( SAMPLING_BUTTON_ID, StringFromFilterMode( mFilterMode ) );
  ...
  bool CheckFittingModeButton( Actor &button, FittingMode::Type mode )
  {
    ...
    {
      mScalingMode = mode;
      SetLabel( mFittingModeButton, modeName );
      ResizeImage();
      mPopup.Hide();
      mPopup.Reset();
      return true;
  ...
  bool CheckSamplingModeButton( Actor &button, SamplingMode::Type mode )
  {
    ...
    {
      mFilterMode = mode;
      SetLabel( mSamplingModeButton, modeName );
      ResizeImage();
      mPopup.Hide();
      mPopup.Reset();
      return true;
  ...
  {
    std::cerr << "pinch.scale: " << pinch.scale << ". pinch.speed: " << pinch.speed << "\n";
  ...
  {
    Stage stage = Stage::GetCurrent();
    std::cerr << "Pan: " << gesture.position << std::endl;
  ...
  void OnKeyEvent(const KeyEvent& event)
  {
    ...
    if( event.state == KeyEvent::Down )
    {
      ...
      else if ( event.keyPressedName == "f" )
      {
        ...
        SetLabel( mSamplingModeButton, StringFromFilterMode( mFilterMode ) );
      }
      // Cycle filter and scaling modes:
      else if ( event.keyPressedName == "s" )
      {
        mScalingMode = NextScalingMode( mScalingMode );
        SetLabel( mFittingModeButton, StringFromScalingMode( mScalingMode ) );
      }
      else
      {
        return;
