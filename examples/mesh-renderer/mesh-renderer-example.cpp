...
public:
  ...
  void LoadScene()
  {
    ...
    layer.RegisterProperty( "Tag", LAYER_TAG ); //Used to differentiate between different kinds of actor.
    layer.TouchedSignal().Connect( this, &MeshRendererController::OnTouch );
    stage.Add( layer );
    ...
    //Intermediate actor to hold light source, to place it in front of everything else.
    Actor placementActor = Actor::New();
    layer.Add( placementActor );
    placementActor.Add( mLightSource );
