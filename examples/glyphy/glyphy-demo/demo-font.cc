...
#include "../glyphy-src/glyphy-freetype.h"
...
demo_font_t *
demo_font_create (const char* path,
                  FT_Face       face,
                  GlyphyAtlas *atlas)
{
                                ...
                                unsigned int requiredWidth )
{
  ...
  Dali::BitmapImage bitmap = Dali::BitmapImage::New( requiredWidth, h, Dali::Pixel::RGBA8888 );
...
{
  std::cout << "_demo_font_upload_glyph index: " << glyph_index << std::endl;
  ...
  {
    //Dali::BitmapImage bitmapImage = CreateBitmap( buffer, output_len, font->atlas->GetItemWidth() );

    Dali::TextAbstraction::FontId fontId = Dali::TextAbstraction::FontClient::Get().GetFontId( font->path );

    Dali::TextAbstraction::GlyphyBlob blob = Dali::TextAbstraction::FontClient::Get().CreateGlyphyBlob( fontId, glyph_index, font->atlas->GetItemWidth() );

    // TODO - Compare blobs

    font->atlas->AtlasAlloc( blob.image, output_len, &glyph_info->atlas_x, &glyph_info->atlas_y);
    //font->atlas->AtlasAlloc( bitmapImage, output_len, &glyph_info->atlas_x, &glyph_info->atlas_y);
  }
}
