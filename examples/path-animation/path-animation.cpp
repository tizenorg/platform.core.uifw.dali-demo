...
public:
  ...
  bool OnTouchPoint(Actor actor, const TouchEvent& event)
  {
    if(event.GetPointCount()>0)
    {
      ...
      if(point.state==TouchPoint::Down) // Start dragging
      {
        mDragActor = actor;
      }
    }
  ...
  bool OnTouchLayer(Actor actor, const TouchEvent& event)
  {
    if(event.GetPointCount()>0)
    {
      ...
      else if(!mDragActor && point.state==TouchPoint::Down && mPath.GetPointCount()<10 ) // Add new point
      {
        const TouchPoint& point = event.GetPoint(0);
...
private:
  ...
  MeshActor mMesh;                    ///< Mesh actor for the segments connecting points and control points
  ImageActor mKnot[10];               ///< ImageActors for the interpolation points
  ImageActor mControlPoint[18];       ///< ImageActors for the control points
