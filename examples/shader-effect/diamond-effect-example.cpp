...
namespace
{
const char * const APPLICATION_TITLE( "Diamond Effect" );
...
{
 DALI_MODEL_DIR "diamond_surface_v01.obj",
 DALI_MODEL_DIR "diamond_surface_v02.obj"
};
...
/************************************************************/
/* Custom diamond effect shader******************************/
/************************************************************/

class DiamondEffect : public ShaderEffect
{
public:

  /**
   * Create an empty DiamondEffect handle.
   */
  DiamondEffect()
  {
  }
   ...
   * Virtual destructor
   */
  virtual ~DiamondEffect()
  {
  }

  /**
   * Create a DiamondEffect object.
   * @return A handle to a newly allocated DiamondEffect
   */
  static DiamondEffect New()
  {
    std::string vertexShader = MAKE_SHADER(
    ...
    std::string fragmentShader = MAKE_SHADER(
      ...
      {
        // pure hue->RGB: RGB=clamp(vec3( abs(hue*6.0-3.0)-1.0, 2.0-abs(hue*6.0-2.0), 2.0-abs(hue*6.0-4.0) ), 0.0, 1.0);\n
    ...
    DiamondEffect handle( shaderEffect );
...
private:
   ...
   * Helper for New()
   */
  DiamondEffect( ShaderEffect handle )
  : ShaderEffect( handle )
  {
  }
};
...
/*************************************************/
/*Demo using DiamondEffect*****************/
/*************************************************/
class DiamondEffectExample : public ConnectionTracker
{
public:
  DiamondEffectExample( Application &application )
    ...
    mCurrentMeshId( 0 )
  {
    ...
    application.InitSignal().Connect(this, &DiamondEffectExample::Create);
  }

  ~DiamondEffectExample()
  {
  }
...
private:
  ...
  void Create(Application& application)
  {
    ...
    stage.KeyEventSignal().Connect(this, &DiamondEffectExample::OnKeyEvent);
    ...
    mChangeTextureButton.ClickedSignal().Connect( this, &DiamondEffectExample::OnChangeTexture );
    ...
    mChangeMeshButton.ClickedSignal().Connect( this, &DiamondEffectExample::OnChangeMesh );
    ...
    mDiamondEffect = DiamondEffect::New(); // used the finger is touching down
    mNoEffect = NoEffect::New(); // used in the other situations, basic render shader, avoids unnecessary calculation
    ...
    CreateDiamondSurface( MESH_FILES[mCurrentMeshId] );

    // Connect the callback to the touch signal on the mesh actor
    mContent.TouchedSignal().Connect( this, &DiamondEffectExample::OnTouch );
    ...
    mLightAnimation.AnimateTo( Property( mDiamondEffect, "uSpinAngle" ), Math::PI*2.f );
  ...
  bool OnTouch( Actor actor , const TouchEvent& event )
  {
    ...
    switch(point.state)
    {
      case TouchPoint::Down:
      {
        ...
        mDiamondEffect.SetLightXYOffset( point.screen - mLightPosition );
        mMeshActor.SetShaderEffect( mDiamondEffect );
        mStrenghAnimation= Animation::New(0.5f);
        mStrenghAnimation.AnimateTo( Property( mDiamondEffect, "uEffectStrength" ), 1.f );
      ...
      case TouchPoint::Interrupted:
      {
        if(mIsDown)
        {
          ...
          mStrenghAnimation = Animation::New(0.5f);
          mStrenghAnimation.AnimateTo( Property( mDiamondEffect, "uEffectStrength" ), 0.f );
          mStrenghAnimation.FinishedSignal().Connect( this, &DiamondEffectExample::OnTouchFinished );
          mStrenghAnimation.Play();
        }
  ...
  void OnTouchFinished( Animation& source )
  {
    ...
    mDiamondEffect.SetLightXYOffset( Vector2::ZERO );
  }

  void CreateDiamondSurface( const std::string& objFileName )
  {
...
{
  DiamondEffectExample theApp(app);
  app.MainLoop();
}
