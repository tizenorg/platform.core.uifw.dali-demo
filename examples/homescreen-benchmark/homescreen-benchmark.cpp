...
public:
  ...
  void Create( Application& application )
  {
    ...
    background.SetSizeScalePolicy(SizeScalePolicy::USE_SIZE_SET);
  ...
  void AddIconsToPage(Actor page)
  {
    ...
    for( int y = 0; y < mConfig.mRows; ++y )
    {
      for( int x = 0; x < mConfig.mCols; ++x )
      {
        ...
        imageView.SetResizePolicy( ResizePolicy::SIZE_RELATIVE_TO_PARENT, Dimension::WIDTH );
        imageView.SetResizePolicy( ResizePolicy::FIXED, Dimension::HEIGHT );
        ...
        imageView.SetSize( Vector3( 0.0f, ROW_HEIGHT * IMAGE_AREA, 1.0f ) );
        ...
        textLabel.SetResizePolicy( ResizePolicy::FIXED, Dimension::ALL_DIMENSIONS );

        textLabel.SetProperty( Toolkit::TextLabel::Property::TEXT_COLOR, Vector4( 1.0f, 1.0f, 1.0f, 1.0f ) );
        textLabel.SetProperty( Toolkit::TextLabel::Property::POINT_SIZE, ( ROW_HEIGHT * LABEL_AREA ) * 0.170f );
...
void RunTest( Application& application, const HomescreenBenchmark::Config& config )
{
...
int main( int argc, char **argv )
{
  ...
  for( int i(1) ; i < argc; ++i )
  {
    ...
    {
      config.mRows = atoi( arg.substr( 2, arg.size() ).c_str() );
    }
    else if( arg.compare( 0, 2, "-c" ) == 0 )
    {
      config.mCols = atoi( arg.substr( 2, arg.size()).c_str() );
    }
    else if( arg.compare( 0, 2, "-p" ) == 0 )
    {
      config.mPageCount = atoi( arg.substr( 2, arg.size() ).c_str() );
    }
