...
const char *FONT_PATH = "/usr/share/fonts/SamsungSans_Rg.ttf";
...
public:
  ...
  void Create( Application& application )
  {
    ...
    // Background
    mStageBackground1 = Dali::Toolkit::CreateSolidColorActor( BACKGROUND_COLOR );
    mStageBackground1.SetAnchorPoint( AnchorPoint::TOP_CENTER );
    mStageBackground1.SetParentOrigin( ParentOrigin::TOP_CENTER );
    mStageBackground1.SetSize( stageSize.x, stageSize.y*0.5f );
    mStageBackground1.SetZ( -0.5f );
    stage.Add(mStageBackground1);
    mStageBackground2 = Dali::Toolkit::CreateSolidColorActor( BACKGROUND_COLOR );
    mStageBackground2.SetAnchorPoint( AnchorPoint::BOTTOM_CENTER );
    mStageBackground2.SetParentOrigin( ParentOrigin::BOTTOM_CENTER );
    mStageBackground2.SetSize( stageSize.x, stageSize.y*0.5f);
    mStageBackground2.SetZ( -1.5f );
    stage.Add(mStageBackground2);
    ...
    demo_buffer_add_text (mBuffer, text, mFont, 1, 0.1);
    ...
    CreateTextView( DEFAULT_SIZE, "Regular",TEXT_COLOR );
    mTextView.SetAnchorPoint( AnchorPoint::CENTER );
    mTextView.SetParentOrigin( ParentOrigin::TOP_LEFT);
    SetTextViewSize( DEFAULT_SIZE, TextStyle::ALL );
	mTextView.SetZ( 0.5f );
    mStageBackground2.Add(mTextView);
    ...
    mPanGestureDetector.Attach( mStageBackground1 );
    mPanGestureDetector.Attach( mStageBackground2 );

    mPinchGestureDetector = PinchGestureDetector::New();
    mPinchGestureDetector.Attach( mActor );
    mPinchGestureDetector.Attach( mStageBackground1);
    mPinchGestureDetector.Attach( mStageBackground2);
    mPinchGestureDetector.DetectedSignal().Connect(this, &GlyphyTextController::OnPinch);

    mTapGestureDetector = TapGestureDetector::New();
    mTapGestureDetector.Attach( mActor );
    mTapGestureDetector.Attach( mStageBackground1 );
    mTapGestureDetector.Attach( mStageBackground2);
...
private:
  ...
  void OnPinch(Actor actor, const PinchGesture& gesture)
  {
    ...
    mSize = clamp(mSizeAtPinchStart * gesture.scale, 2.f, 500.f);
    mActor.SetScale( mSize );

    SetTextViewSize( mSize, TextStyle::SIZE);
  }
  ...
  void OnTap( Actor actor, const TapGesture& tapGesture )
  {
    ...
    else if( actor == mActor || actor == mTextView || actor == mStageBackground1 || actor == mStageBackground2)
    {
      HideOptionPanel();
    }
  }
  ...
  void Reset()
  {
    ...
    SetTextViewSize(mSize, TextStyle::SIZE);
    mTextView.SetPosition( Vector3(mCompensateMovement));
  }
  ...
  void CreateTextView( int pixelSize, const std::string& style, const Vector4& textColor)
                    //bool shadow = false, const Vector4& shadowColor = Color::BLACK, const Vector2& shadowOffset = Vector2::ZERO, const float shadowSize = 0.0f )
  {
   ...
   // mTextStyle.SetShadow( shadow, shadowColor, shadowOffset, shadowSize );
  ...
  void SetTextViewSize( float size, TextStyle::Mask mask )
  {
    if( fabs(size - mFontSize) < 5.f  )
    {
      return;
    }
    mFontSize = size;
	size = clamp( Font::PixelsToPoints(static_cast<unsigned int>( size+0.5f ) ), 2.f, 500.f);
    mTextStyle.SetFontPointSize( PointSize( size ) );
    mTextView.SetStyleToCurrentText( mTextStyle, mask );
    TextView::TextLayoutInfo textLayoutInfo;
    mTextView.GetTextLayoutInfo(textLayoutInfo);
    mTextView.MoveBy( Vector3(textLayoutInfo.mTextSize*0.5f - mCompensateMovement)  );
...
private:
  ...
  MeshActor        mActor;
  ImageActor       mStageBackground1;
  ImageActor       mStageBackground2;
