...
Atlas LoadImage( const char*imagePath )
{
...
Atlas LoadImage( const char*imagePath1, const char* imagePath2 )
{
  BitmapLoader loader1 = BitmapLoader::New( imagePath1 );
  loader1.Load();
  PixelData pixelData1 = loader1.GetPixelData();
  BitmapLoader loader2 = BitmapLoader::New( imagePath2 );
  loader2.Load();
...
class PortraitPageFactory : public PageFactory
{
  ...
  virtual Image NewPage( unsigned int pageId )
  {
    ...
    if( pageId == 0 )
    {
      page = LoadImage( BOOK_COVER_PORTRAIT );
    }
    else
    {
      page = LoadImage( PAGE_IMAGES_PORTRAIT[ (pageId-1) % NUMBER_OF_PORTRAIT_IMAGE ] );
    }
...
class LandscapePageFactory : public PageFactory
{
  ...
  virtual Image NewPage( unsigned int pageId )
  {
    ...
    if( pageId == 0 )
    {
      page = LoadImage( BOOK_COVER_LANDSCAPE, BOOK_COVER_BACK_LANDSCAPE );
    }
    else
    {
      unsigned int imageId = (pageId-1)*2;
      page = LoadImage( PAGE_IMAGES_LANDSCAPE[ imageId % NUMBER_OF_LANDSCAPE_IMAGE ], PAGE_IMAGES_LANDSCAPE[ (imageId+1) % NUMBER_OF_LANDSCAPE_IMAGE ] );
    }
