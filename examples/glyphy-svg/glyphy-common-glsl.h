...
\n
int winding_arc( const glyphy_arc_t arc, const vec2 arc_center, const vec2 p, const vec2 cell_center)\n
{\n
  ...
  if (bb4ac > 0.) {\n
    ...
    if( mu1 == clamp(mu1, 0., 1.) && glyphy_arc_wedge_contains( arc, mix(p, cell_center, mu1) ) )\n
        count += 1;\n
    if( mu2 == clamp(mu2, 0., 1.) && glyphy_arc_wedge_contains( arc, mix(p, cell_center, mu2) ) )\n
        count += 1;\n
  }\n
  return count;\n
}\n
