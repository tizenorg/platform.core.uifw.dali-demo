...
void
demo_buffer_add_svg_shape(demo_buffer_t        *buffer,
              NSVGshape  *svgShape,
              demo_svg_t *svg)
{
  glyph_info_t gi;
  demo_svg_shape_upload (svg, svgShape, &gi);

  /* Update ink extents */
  glyphy_extents_t ink_extents;
  demo_add_glyph_vertices (buffer->cursor, 1, &gi, buffer->vertices, &ink_extents);
