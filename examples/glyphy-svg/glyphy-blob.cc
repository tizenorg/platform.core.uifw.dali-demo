...
arc_endpoint_encode (unsigned int ix, unsigned int iy, double d)
{
  ...
  v.r = id;
  v.g = LOWER_BITS (ix, 8, 11);
  v.b = LOWER_BITS (iy, 8, 11);
  v.a = ((ix >> 8) << 4) | (iy >> 8);
  return v;
}
...
{
  glyphy_rgba_t v;
  v.r = UPPER_BITS (num_points, 8, 15); // unused for arc-list encoding
			     ...
			     glyphy_extents_t            *pextents)
{
  ...
  unsigned int grid_size = GRID_SIZE < num_endpoints ? GRID_SIZE : num_endpoints;
