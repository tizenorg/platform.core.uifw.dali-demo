...
{
 // if( num_points>10 ) std::cout<<"num_points_per_cell: "<<num_points<<std::endl;
  glyphy_rgba_t v;
 // v.r = LOWER_BITS (count, 8, 15);
  int winding = count % 2;
			     ...
			     glyphy_extents_t            *pextents)
{
 ...
 // unsigned int unit_int = static_cast<unsigned int>(unit/5.0+0.5);

  unsigned int num_size = sqrt(num_endpoints/2);
  unsigned int grid_size = GRID_SIZE < num_size ? GRID_SIZE : num_size;
 // grid_size = unit_int < grid_size ? unit_int : grid_size;
