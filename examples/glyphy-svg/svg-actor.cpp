...
Shader SVGActor::CreateShader()
{
  ...
  vertexShaderStringStream<<"precision highp float;\n"
                          <<"precision highp int;\n"
                          <<vertexShaderSource;
  ...
  std::string enableExtensionString("#extension GL_OES_standard_derivatives : enable\nprecision highp float;\nprecision highp int;\n");

  Shader shader = Shader::New( vertexShaderStringStream.str(),
...
void SVGActor::Initialize()
{
  ...
  // register uniforms
  mIndexAtlasInfo = mActor.RegisterProperty( "uAtlasInfo", Vector4( 1024.f, 1024.f, 64.f, 8.f ) );
  mIndexStrokeScale = mActor.RegisterProperty( "uStrokeScale", 1.f );
  mIndexGradientCount = mActor.RegisterProperty( "uGradientCount", 0.f );
}
...
void SVGActor::SetVertices( std::vector<Vertex>* vertexData )
{
  ...
  vertexFormat["aStrokeColot"] = Property::VECTOR4;
...
void SVGActor::SetGlyphyAtlas( BufferImage glyphyAtlas )
{
  Sampler sampler = Sampler::New( glyphyAtlas, "sGlyphyAtlas" );
  mMaterial.AddSampler( sampler );
}

void SVGActor::SetGradientLookupTexture( BufferImage lookupTexture )
{
  Sampler sampler = Sampler::New( lookupTexture, "sGradientLookup" );
  mMaterial.AddSampler( sampler );
}

void SVGActor::SetAtlasInfo( const Vector4& atlasInfo )
{
  mActor.SetProperty( mIndexAtlasInfo, atlasInfo );
}

void SVGActor::SetGradientCount( int count )
{
  mActor.SetProperty( mIndexGradientCount, static_cast<float>(count) );
}
