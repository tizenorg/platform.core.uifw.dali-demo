...
const char* SVG_IMAGES[] =
{
  ...
  DALI_IMAGE_DIR "icon_green.svg",
  DALI_IMAGE_DIR "blue_icon.svg",
...
public:
  ...
  MeshActor CreateMeshActorFromSVG( const char* svgFile )
  {
    ...
    {
      unsigned int fill_color = 0xFF000000;
      char type = (shape->fill).type;//fill_solid_color
      ...
      else if( type == 2 || type == 3) // fill_gradient
      {
        ...
        if( gradient->units ==  0) // User Space
        {
          float bounds[] = {0.f,0.f, 1.f, 1.f};
          pack_gradient_info( gradientInfo, type, bounds, gradient->xform );
        }
        else // Bounding Box Space
        {
          pack_gradient_info( gradientInfo, type, shape->bounds, gradient->xform );
        }
  ...
  float pack_opacity(unsigned int gradientIndex, unsigned int fillColor, unsigned int strokeColor, float shapeOpacity)
  {
    ...
    unsigned int fillOpacity = static_cast<float>(fillColor >> 24) * shape_opacity;
    unsigned int strokeOpacity = static_cast<float>( strokeColor >> 24 ) * shape_opacity;
  ...
  void pack_gradient_info( Dali::Vector<float>& gradientInfo, char type, float* bounds, float* xform )
  {
    float a = 0.f;
    float b = 0.f;
    float c = 0.f;
    ...
    {
      a = xform[0] / (bounds[2] - bounds[0]);
      b = xform[2] / (bounds[3] - bounds[1]);
      c = xform[4] - xform[0]*bounds[0]/(bounds[2]-bounds[0]) - xform[2]*bounds[1]/(bounds[3]- bounds[1]);
    }
    float e = xform[1] / (bounds[2] - bounds[0]);
    float f = xform[3] / (bounds[3] - bounds[1]);
    float g = xform[5] - xform[1]*bounds[0]/(bounds[2]-bounds[0]) - xform[3]*bounds[1]/(bounds[3]- bounds[1]);
    gradientInfo.PushBack(a);
    gradientInfo.PushBack(b);
    gradientInfo.PushBack(c);
    gradientInfo.PushBack(e);
    gradientInfo.PushBack(f);
    gradientInfo.PushBack(g);
  }
