...
public:
  ...
  MeshActor CreateMeshActorFromSVG( const char* svgFile )
  {
    ...
    for (NSVGshape* shape = image->shapes; shape != NULL; shape = shape->next)
    {
      ...
      else if( type == 2 ) // fill_linear_gradient
      {
        NSVGgradient* gradient = (shape->fill).gradient;
        if( gradient->units ==  0) // User Space
        {
          float bounds[] = {0.f,0.f, 1.f, 1.f};
          pack_linear_gradient_info( gradientInfo, bounds, gradient->xform );
        }
        else // Bounding Box Space
        {
          pack_linear_gradient_info( gradientInfo, shape->bounds, gradient->xform );
        }
      ...
      else if( type == 3 )// fill_radial_gradient
      {
        //Todo: handle radial gradient
      }
  ...
  void pack_linear_gradient_info( Dali::Vector<float>& gradientInfo, float* bounds, float* xform )
  {
    float a = xform[1] / (bounds[2] - bounds[0]);
    float b = xform[3] / (bounds[3] - bounds[1]);
    float c = xform[5] - xform[1]*bounds[0]/(bounds[2]-bounds[0]) - xform[3]*bounds[1]/(bounds[3]- bounds[1]);
