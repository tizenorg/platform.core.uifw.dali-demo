...
\n
  v_gradient_type = 0.;\n
  if( aGradientIndex > 0.5 ){
    v_texture_coord.y = aGradientParamY[0]*aPosition.x
                      - aGradientParamY[1]*aPosition.y
                      + aGradientParamY[2];\n
    if( abs(aGradientParamX[0])+abs(aGradientParamX[1])+ abs(aGradientParamX[2])< 1e-5 ){\n
       // linear gradient
      v_texture_coord.x = 0.;\n
      v_gradient_type = 1.; }\n
    else{\n // radial gradient
      v_texture_coord.x = aGradientParamX[0]*aPosition.x-aGradientParamX[1]*aPosition.y+aGradientParamX[2];\n
      v_gradient_type = 2.; }\n
    v_texture_coord.z = ( aGradientIndex - 0.5 )/uGradientCount;\n
  }\n
}\n
);
