  ...
  for( int row = 0; row<grid_h; row++ )
    for( int col = 0; col<grid_w; col++ )
    {
      ...
      if( pivots[idx] )
      {
        ...
        double y = extents->min_y + (row+0.7) * cell_height;
        for( unsigned int i=0; i<num_endpoints; i++ ){
          const glyphy_arc_endpoint_t &endpoint = endpoints[i];
          if (endpoint.d == GLYPHY_INFINITY)
          {
          ...
          Point current(extents->min_x+(col+0.3)*cell_width,  y);
    ...
    for( int col = 0; col<grid_w; col++ )
    {
      ...
      if( pivots[idx] > 1)
      {
        ...
        for( unsigned int i=0; i<num_endpoints; i++ ){
          ...
          Point current(extents->min_x+(col+0.7)*cell_width,  y);
          if( fabs(arc.distance_to_point( current )) < threshold*cell_width  )
          {
            pivots[ idx] = 3;
            counts[idx] = 0;
          ...
          if( iszero (arc.d) ) // line
          {
            ...
            if( found )
            {
              float location = (x-extents->min_x) / cell_width  + 0.3;
              if(col < int(location) )
              {
                count++;
              }
          ...
          else // arc
          {
            ...
            if( found1 )
            {
              float location = (x1-extents->min_x) / cell_width  + 0.3;
              if(col < int(location) )
              {
                count++;
              }
            ...
            if( found2 )
            {
              float location = (x2-extents->min_x) / cell_width  + 0.3;
              if(col < int(location) )
              {
                count++;
              }
