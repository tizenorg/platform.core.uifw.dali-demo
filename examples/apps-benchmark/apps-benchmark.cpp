...
const float kPageScaleFactorY = 0.95f;
const float kPageScaleFactorX = 0.95f;
const float kPageDurationScaleFactor = 1.0f;
}

// This example shows how to create and display Hello World! using a simple TextActor
//
class AppsBenchmarkController : public ConnectionTracker
{
public:
  ...
  AppsBenchmarkController( Application& application, int rows, int cols, int pages )
  : mApplication( application )
  {
    ...
    mApplication.InitSignal().Connect( this, &AppsBenchmarkController::Create );
  }

  ~AppsBenchmarkController()
  {
    // Nothing to do here;
  }
  ...
  void Create( Application& application )
  {
    ...
    stage.GetRootLayer().TouchedSignal().Connect( this, &AppsBenchmarkController::OnTouch );
  }
  ...
  Actor AddPage()
  {
    ...
    Actor pageActor = Toolkit::CreateSolidColorActor( Vector4( 0.0f, 0.0f, 0.0f, 0.5f ) );
    ...
    pageActor.SetSizeModeFactor(Vector3( kPageScaleFactorX, kPageScaleFactorY, 1.0f ) );
    return pageActor;
  }
  ...
  void AddIconsToPage(Actor page)
  {
    ...
    const float scaledHeight = stageSize.y * kPageScaleFactorY;
    const float scaledWidth = stageSize.x * kPageScaleFactorX;

    const float padding = 8.0f;
    const float kRowHeight = ( scaledHeight - (padding*2.0f) ) / (float)mRows;
    const float kRowWidth = ( scaledWidth - (padding*2.0f) ) / (float)mCols;

    const float kImageArea = 0.60f;
    const float kLabelArea = 0.40f;

    static int currentIconIndex = 0;

    for( int y = 0; y < mRows; ++y )
    {
      float rowY = y * kRowHeight + padding;
      for( int x = 0; x < mCols; ++x )
      {
        float rowX = x * kRowWidth + padding;

        // create image view
        Toolkit::ImageView iv = Toolkit::ImageView::New( IMAGE_PATH[currentIconIndex] );
        iv.SetSizeScalePolicy( SizeScalePolicy::FIT_WITH_ASPECT_RATIO );
        iv.SetAnchorPoint( AnchorPoint::TOP_LEFT );
        iv.SetParentOrigin( ParentOrigin::TOP_LEFT );
        page.Add( iv );
        iv.SetSize( Vector3(kRowWidth, kRowHeight*kImageArea, 1.0f) );
        iv.SetPosition( Vector3(rowX, rowY, 0.0f) );

        // create label
        Toolkit::TextLabel textLabel = Toolkit::TextLabel::New( DEMO_APPS_NAMES[currentIconIndex] );
        textLabel.SetSizeScalePolicy( SizeScalePolicy::FIT_WITH_ASPECT_RATIO );
        textLabel.SetAnchorPoint( AnchorPoint::TOP_CENTER );
        textLabel.SetParentOrigin( ParentOrigin::TOP_LEFT );
        textLabel.SetProperty( Toolkit::TextLabel::Property::TEXT_COLOR, Vector4(1.0, 1.0, 1.0, 1.0));
        textLabel.SetProperty( Toolkit::TextLabel::Property::POINT_SIZE, 10);
        textLabel.SetProperty( Toolkit::TextLabel::Property::HORIZONTAL_ALIGNMENT, "CENTER");
        page.Add( textLabel );
        textLabel.SetSize( Vector3(kRowWidth, kRowHeight*kLabelArea, 1.0f) );
        textLabel.SetPosition( Vector3(rowX + (kRowWidth*0.5f), (rowY+kRowHeight)-(kLabelArea*kRowHeight), 0.0f) );
  ...
  void CreateScript()
  {
    const int lastPage = mPageCount-1;
  ...
  void PopulatePages()
  {
    ...
    for( int i = 0; i < mPageCount; ++i )
    {
      ...
      page.SetPosition( Vector3(stageSize.x*i, -0.30f*( stageSize.y-scaledHeight ), 0.0f) );
  ...
  void ShowAnimation()
  {
    ...
    mShowAnimation.FinishedSignal().Connect( this, &AppsBenchmarkController::OnAnimationEnd );
    mShowAnimation.Play();
  }

  void ScrollPages(int pages, float duration, bool flick)
  {
    duration *= kPageDurationScaleFactor;
    Vector3 stageSize( Stage::GetCurrent().GetSize() );
    ...
    mScrollAnimation.FinishedSignal().Connect( this, &AppsBenchmarkController::OnAnimationEnd );
...
void RunTest( Application& application, int rows, int cols, int pages )
{
  AppsBenchmarkController test( application, rows, cols, pages );
...
int main( int argc, char **argv )
{
  ...
  for( int i(1) ; i < argc; ++i )
  {
    ...
    {
      rows = atoi( arg.substr( 2, arg.size()).c_str());
    }
    else if( arg.compare(0, 2, "-c" ) == 0)
    {
      cols = atoi( arg.substr( 2, arg.size()).c_str());
    }
    else if( arg.compare(0, 2, "-p" ) == 0)
    {
      pages = atoi( arg.substr( 2, arg.size()).c_str());
    }
  }

  RunTest( application, rows, cols, pages );
