...
public:
  ...
  Actor AddPage()
  {
    ...
    if( gUseTableView )
    {
      ...
      Toolkit::TableView tableView = Toolkit::TableView::New( mRows, mCols );
      tableView.SetBackgroundColor( Vector4( 0.0f, 0.0f, 0.0f, 0.5f ) );
      pageActor = tableView;
    }
    else
    {
      pageActor = Toolkit::CreateSolidColorActor( Vector4( 0.0f, 0.0f, 0.0f, 0.5f ) );
    }
  ...
  void AddIconsToPage(Actor page)
  {
    ...
    for( int y = 0; y < mRows; ++y )
    {
      for( int x = 0; x < mCols; ++x )
      {
        ...
        if( gUseTableView )
        {
          imageView.SetPosition( Vector3( 0.0f, PADDING, 0.0f) );
        }

        // create label
        Toolkit::TextLabel textLabel = Toolkit::TextLabel::New( DEMO_APPS_NAMES[currentIconIndex] );
        textLabel.SetAnchorPoint( AnchorPoint::BOTTOM_LEFT );
        textLabel.SetParentOrigin( ParentOrigin::BOTTOM_LEFT );

        textLabel.SetResizePolicy( ResizePolicy::SIZE_RELATIVE_TO_PARENT, Dimension::WIDTH);
        ...
        textLabel.SetSize( Vector3( 0.0f, ROW_HEIGHT*LABEL_AREA, 1.0f) );
        textLabel.SetPosition( Vector3( 0.0f, 0.0f , 0.0f) );

        iconView.Add( imageView );
        iconView.Add( textLabel );
