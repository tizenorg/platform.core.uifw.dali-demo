...
const float PAGE_DURATION_SCALE_FACTOR( 1.0f );
}
...
public:
  ...
  void AddIconsToPage(Actor page)
  {
    ...
    const float scaledHeight = stageSize.y * PAGE_SCALE_FACTOR_X;
    const float scaledWidth = stageSize.x * PAGE_SCALE_FACTOR_Y;

    const float PADDING = stageSize.y / 64.0f;
    const float ROW_HEIGHT = ( scaledHeight - (PADDING*2.0f) ) / (float)mRows;
    const float COL_WIDTH = ( scaledWidth - (PADDING*2.0f) ) / (float)mCols;

    const float IMAGE_AREA = 0.60f;
    const float LABEL_AREA = 0.40f;
    ...
    for( int y = 0; y < mRows; ++y )
    {
      ...
      for( int x = 0; x < mCols; ++x )
      {
        ...
        page.Add( iv );
        iv.SetSize( Vector3(COL_WIDTH - 2*PADDING, ROW_HEIGHT*IMAGE_AREA, 1.0f) );
        ...
        textLabel.SetProperty( Toolkit::TextLabel::Property::POINT_SIZE, stageSize.y / 64.0f);
        textLabel.SetProperty( Toolkit::TextLabel::Property::HORIZONTAL_ALIGNMENT, "CENTER");
        page.Add( textLabel );
        textLabel.SetSize( Vector3(COL_WIDTH, ROW_HEIGHT*LABEL_AREA, 1.0f) );
        textLabel.SetPosition( Vector3(rowX - PADDING, (rowY+ROW_HEIGHT)-(LABEL_AREA*ROW_HEIGHT), 0.0f) );
