 ...
 *
 * Uses ShrinkToFit which ensures the resulting image is
...
ResourceImage LoadImageFittedInBox( const char * const imagePath, uint32_t maxWidth, uint32_t maxHeight )
{
  ...
  return ResourceImage::New( imagePath, ImageDimensions( maxWidth, maxHeight ), ScalingMode::ShrinkToFit, Dali::SamplingMode::BoxThenLinear );
}
