...
void main()
{
  ...
  vertexPosition.xyz *= 160.0; //uSize;
  vertexPosition = uMvpMatrix * vertexPosition;

  //vGlobColor = vec3(1.0,1.0,1.0);  //hsv2rgb( vec3( clamp(uHue, 0.0, 1.0), 0.5, 0.5 ) );
...
void main()
{
  gl_FragColor = uColor;//texture2D( sTexture, vTexCoord ) * uColor;// * vec4(vGlobColor, 1.0) ;
}
);
...
public:
  ...
  void Create( Application& application )
  {
    ...
    for(unsigned int i=0; i<numMeshes; i++)
    {
      ...
      material.SetProperty(Material::Property::COLOR, Vector4( 0.5f+(0.5f*i/numMeshes), 0.5f, 0.5f, 1.0f));

      Renderer renderer = Renderer::New( mGeometry, material );
      Actor meshActor = Actor::New();
      meshActor.AddRenderer( renderer );
      meshActor.SetSize( 160, 160 );

      // Test with actor alpha
      meshActor.SetParentOrigin( ParentOrigin::TOP_LEFT );
      meshActor.SetAnchorPoint( AnchorPoint::TOP_LEFT );

      float offset = (stage.GetSize().width - 50.0f) / numMeshes;
      meshActor.SetPosition( 50 + i*offset, 50 + i*offset, 0 );
      meshActor.SetColor( Vector4( 0.5f + (0.5f * i/numMeshes), 1.0f - (0.5f * i/numMeshes), 0.5f, 1.0f ) );
   ...
   * @param[in] application The application instance
   */
  void OldCreate( Application& application )
  {
    Stage stage = Stage::GetCurrent();
    stage.KeyEventSignal().Connect(this, &ExampleController::OnKeyEvent);
    ...
    for( unsigned i=0; i<NUMBER_OF_SAMPLES; ++i)
    {
      ...
      meshActor.SetPosition( -100 + 40.0f*i, -100 + 40.0f*i, i*10 );

      meshActor.SetOpacity( 0.5f );
