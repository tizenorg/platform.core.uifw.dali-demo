...
struct BasicTextRenderer::AtlasHelper
{
  ...
  void AddGlyph( GlyphInfo& glyphInfo )
  {
    ...
    {
      mAtlasHelperSize.height = glyphInfo.height + PADDING;
    }
  }

  Atlas CreateAtlas() const
  {
...
Atlas BasicTextRenderer::CreateAtlas( const Vector<GlyphInfo>& glyphs )
{
  ...
  for( unsigned int i=0; i<glyphs.Count(); ++i )
  {
    ...
    {
      if( !mAtlasHelper->GlyphFound( glyph.fontId, glyph.index ) )
      {
        mAtlasHelper->AddGlyph( glyph );
      }
    }
  }
