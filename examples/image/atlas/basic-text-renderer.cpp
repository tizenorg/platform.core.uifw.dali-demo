...
struct BasicTextRenderer::AtlasHelper
{
  ...
  Atlas CreateAtlas()
  {
    ...
    for( unsigned int i=0; i<mGlyphs.size(); ++i )
    {
      ...
      coords.topLeft.x     = glyph.xOffset / mWidth;
      coords.topLeft.y     = 0.0f;
      coords.topRight.x    = (glyph.xOffset + glyph.width) / mWidth;
      coords.topRight.y    = 0.0f;
      coords.bottomLeft.x  = glyph.xOffset / mWidth;
      coords.bottomLeft.y  = glyph.height / mHeight;
      coords.bottomRight.x = (glyph.xOffset + glyph.width) / mWidth;
      coords.bottomRight.y = glyph.height / mHeight;
    }
...
RenderableActor BasicTextRenderer::Render( Text::ViewInterface& view )
{
  ...
  if( numberOfGlyphs > 0 )
  {
    ...
    //ImageActor actor = ImageActor::New( atlas );
